#!/usr/bin/env bash
# 운영 서버 부트스트랩 — 새 호스트를 배포 가능 상태로 만든다
#
# 사용법: ./scripts/bootstrap.sh
#         운영 서버 주소와 도메인을 순서대로 물어본다. 비대화형으로 돌리려면
#         환경변수를 미리 설정한다 — ssh config 는 필요하지 않다:
#
#         SSH_KEY=~/.ssh/your-key SSH_USER=ubuntu HOST=203.0.113.10 \
#           DOMAIN=quant.example.com ./scripts/bootstrap.sh
#
#   DOMAIN        서비스 도메인 (예: quant.example.com). A 레코드가 운영 서버의 고정 공인
#                    IP 를 가리키고 있어야 한다 — Caddy 가 이 이름으로 인증서를 받는다.
#   HOST      운영 서버 주소, `[user@]host` 형식
#   SSH_USER      로그인 사용자명. HOST 에 `user@` 가 없을 때만 쓴다
...
TARGET="${HOST:-}"
if [ -z "${TARGET}" ]; then
  read -rp "운영 서버 주소 입력(user@host): " TARGET || true
fi
[ -n "${TARGET}" ] || {
  echo "운영 서버 주소가 필요합니다 — 비대화형이면 HOST 로 지정하세요" >&2
  exit 1
}
# 사용자명은 주소에 붙여도 되고 따로 줘도 된다 — 주소가 IP 뿐인 CI·스크립트에서
# HOST 를 조립하지 않아도 되게 한다. 둘 다 있으면 주소에 붙은 쪽이 이긴다.
if [ -n "${SSH_USER:-}" ]; then
  case "${TARGET}" in
...
case "${TARGET}" in
  -* | *[[:space:]]*)
    echo "운영 서버 주소 형식이 올바르지 않습니다: ${TARGET}" >&2
    exit 1
    ;;
esac
...
LOG="${LOG:-${REPO_ROOT}/.logs/bootstrap-$(date -u +%Y%m%d-%H%M%S).log}"
mkdir -p "$(dirname "${LOG}")"
exec > >(tee "${LOG}") 2>&1
TEE_PID=$!
...
if [ -n "${SSH_KEY:-}" ]; then
  ...
  # agent 에 등록된 키까지 순서대로 제시되면 맞는 키가 4번째가 되어 운영 서버가 먼저 끊는다.
        ...
        if [ -n "${SSH_KEY:-}" ]; then
          ...
          if [ -n "${CANDIDATES}" ]; then
            ...
            while IFS= read -r k; do
              [ -n "${k}" ] && echo "  SSH_KEY=${k} HOST=${TARGET} ./scripts/bootstrap.sh"
            done <<< "${CANDIDATES}"
          else
            echo
...
# 인증서의 확정 판정은 여기다 — 운영 서버 안(hairpin 제약 가능)이 아니라 외부 시점.
...
if [ "${CODE}" = "000" ] || [ -z "${CODE}" ]; then
  ...
  echo "  - A 레코드가 운영 서버 고정 IP 를 가리키는지, DNS 전파가 끝났는지 확인" >&2
  echo "  - 클라우드 방화벽에서 TCP 80·443 이 열려 있는지 확인" >&2
  echo "  - 운영 서버에서: journalctl -u caddy --no-pager -n 50" >&2
  exit 1
fi
...
다음 단계:
  1) deploy.env 작성 후 앱과 에이전트 배포 파일 게시: pnpm run deploy
  2) 관리자 생성 + TOTP 등록 (운영 서버에서, 순서대로):
