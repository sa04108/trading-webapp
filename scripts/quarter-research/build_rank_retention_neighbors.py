"""보유 순위 완충 후보의 단일 변수 이웃을 같은 날짜 기본 계좌와 대조한다."""
...
def build(root, destination):
    ...
    experiments_path, base_path = cfg / "rank-retention-neighbors-experiments.json", cfg / "rank-retention-20.json"
    protocol = Path("docs/research/kr-current-quarter-rank-retention-neighbors-protocol.md")
    ...
    variants = {"formation21": ({"formationDays": 21}, 5, 20), "formation25": ({"formationDays": 25}, 5, 20),
                "top4": ({"topN": 4}, 4, 20), "top6": ({"topN": 6}, 6, 20), "retention7": ({"retentionRank": 7}, 5, 20), "retention12": ({"retentionRank": 12}, 5, 20)}
    ...
    for stage in ("validation", "confirmation"):
        ...
        windows, checked = verify(root, directory, base_candidate, stage, options_path, input_path, 15, options["starts"], strategy_version=version,
                                  expected_parameters=base_candidate["parameters"] | {"absoluteMomentumFilter": True})
    ...
    for e in experiments:
        ...
        expected = base_candidate | {"id": "rank-retention-neighbor-" + e["variant"], "parameters": base_candidate["parameters"] | modified_parameters,
    ...
    flat = [{k: r[k] for k in ("variant", "stage", "reused", "passes")} | {"formationDays": r["candidate"]["parameters"]["formationDays"],
             "topN": r["candidate"]["topN"], "rebalanceBars": r["candidate"]["rebalanceBars"], "retentionRank": r["candidate"]["parameters"]["retentionRank"]} | r["all"]
    ...
    for r in records:
        ...
        for w in nonoverlapping(r["windows"]):
            path = root / experiment["directory"] / f'{r["candidate"]["id"]}__{w["start"]}.json'
            base_run_path = root / f'rank-retention/20-{r["stage"]}' / f'{base_candidate["id"]}__{w["start"]}.json'
    ...
    ranked_pairs = [(r, p) for r in records if not r["reused"] for p in r["paired"]]
    extremes = []
    for label, field, largest in (("worstReturn", "returnPct", False), ("worstDrawdown", "drawdownPct", False),
                                  ("largestReturnDecrease", "returnDeltaPctPoints", False),
                                  ("largestReturnIncrease", "returnDeltaPctPoints", True)):
        r, pair = (max if largest else min)(ranked_pairs, key=lambda item: item[1][field])
        e = next(e for e in experiments if e["variant"] == r["variant"] and e["stage"] == r["stage"])
        path = root / e["directory"] / f'{r["candidate"]["id"]}__{pair["start"]}.json'
        base_run_path = root / f'rank-retention/20-{r["stage"]}' / f'{base_candidate["id"]}__{pair["start"]}.json'
        extremes.append({"selection": label, "variant": r["variant"], "stage": r["stage"], "paired": pair,
                         "base": trade_path(base_run_path), "neighbor": trade_path(path)})
    write("extreme-account-audit.json", extremes)
    ...
    code += [Path("tests/unit") / f"{name}.test.ts" for name in ("rank-retention-quarter", "uncertain-history-quarter", "confirmed-entry-quarter", "recovery-quarter", "cross-sectional-momentum")]
    ...
    manifest = {"asof": "2026-09-08", "baseCommit": "c034d7c", "newExperiments": 12,
