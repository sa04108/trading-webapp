"""종목 손절만 바꾼 고정 실험과 재사용한 기본 계좌를 대조한다."""
...
def build(root, destination):
    ...
    experiments = json.loads((config / "position-stop-experiments.json").read_text())
    ...
    for experiment in experiments:
        ...
        for window in trial["windows"]:
            ...
            expected_parameters = {"formationDays": 60, "topN": 3, "targetAnnualVolatility": .3,
                                   "positionStopPct": candidate["parameters"]["positionStopPct"]}
        ...
        records.append({"stopPct": candidate["parameters"]["positionStopPct"], "stage": trial["stage"], "reused": False,
                        "directory": experiment["directory"], "all": trial["all"], "nonoverlapping": trial["nonoverlapping"], "windows": trial["windows"]})
    for stage, directory in (("development", "entry/monthly-development"), ("validation", "entry/daily-validation"), ("confirmation", "entry/daily-confirmation")):
        path = root / directory / "recovery-60-3-monthly__summary.json"
        ...
        if [w["start"] for w in windows] != [w["start"] for w in next(r for r in records if r["stage"] == stage)["windows"]]:
            raise ValueError("기본 후보와 손절 변경 후보의 시작점이 다릅니다")
        for window in windows:
            run_path = path.with_name(f'recovery-60-3-monthly__{window["start"]}.json')
            run = json.loads(run_path.read_text())
            if (run["summary"] != window or run["parameters"] != {"formationDays": 60, "topN": 3, "targetAnnualVolatility": .3, "positionStopPct": 8}
        ...
        sources.add(path)
        records.append({"stopPct": 8, "stage": stage, "reused": True, "directory": directory,
                        "all": summarize(windows), "nonoverlapping": summarize(nonoverlapping(windows)), "windows": windows})
    records.sort(key=lambda r: (r["stopPct"], {"development": 0, "validation": 1, "confirmation": 2}[r["stage"]]))
    ...
    manifest = {"asof": "2026-09-08", "baseCommit": "04eed6d", "newExperiments": len(experiments),
