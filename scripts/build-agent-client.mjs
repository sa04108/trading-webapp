...
try {
  ...
  const entries = AGENT_ENTRYPOINTS.map((file) =>
  ...
  for (const file of graph.files.keys()) {
    if (!/^dist\/(agent|runtime|shared)\//.test(file))
      throw new Error(`서버 전용 코드가 agent 패키지에 들어왔습니다: ${file}`);
  }
  ...
  function prune(directory) {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) prune(file);
        ...
        !graph.files.has(
          path.relative(temporary, file).replaceAll(path.sep, "/"),
        )
      )
        fs.unlinkSync(file);
    }
  ...
  for (const file of ["package.json", "pnpm-lock.yaml", "pnpm-workspace.yaml"])
    fs.copyFileSync(
      path.join(root, "packages/agent", file),
  ...
  fs.copyFileSync(
    ...
    path.join(temporary, "dist/build-info.json"),
  );
  fs.writeFileSync(
    path.join(temporary, "dist/runtime-versions.json"),
    JSON.stringify(versions, null, 2),
  );
  fs.cpSync(
    path.join(root, "migrations/agent"),
    path.join(temporary, "migrations/agent"),
    { recursive: true },
  );
  ...
  for await (const chunk of fs.createReadStream(stagedArchive))
    hash.update(chunk);
  ...
  const previous = fs.existsSync(manifestPath)
    ...
    : { clients: [] };
  const clients =
    previous.runnerVersion === versions.agentVersion
      ? previous.clients.filter((client) => client.arch !== process.arch)
      : [];
  clients.push({
    buildGitSha: metadata.gitSha,
    arch: process.arch,
    file,
    sha256,
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        runnerVersion: versions.agentVersion,
        buildGitSha: metadata.gitSha,
        clients,
      },
      null,
