...
const dependencies = Object.fromEntries(
  names.map((name) => [name, selected.roots[name].split("(")[0]]),
);
const manifest = {
  name: "quant-agent-runtime",
  version: "0.0.0",
  private: true,
  type: "module",
...
fs.writeFileSync(
  new URL("../packages/agent/pnpm-lock.yaml", import.meta.url),
  yaml.dump(
    {
      ...
      importers: {
        ".": {
          dependencies: Object.fromEntries(
            names.map((name) => [
              name,
              { specifier: dependencies[name], version: selected.roots[name] },
            ]),
          ),
        },
      },
