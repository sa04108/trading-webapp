...
#
# 도메인의 A 레코드가 이 운영 서버의 고정 공인 IP 를 가리켜야 한다 — Caddy 가 그 이름으로
...
[ -n "${DOMAIN}" ] || {
  ...
  echo "도메인의 A 레코드가 이 운영 서버의 고정 공인 IP 를 가리키고 있어야 합니다." >&2
  exit 1
}
...
if [ -z "${CODE}" ] || [ "${CODE}" = "000" ]; then
  echo "경고: https://${DOMAIN} 의 TLS 응답을 운영 서버 안에서 확인하지 못했다." >&2
