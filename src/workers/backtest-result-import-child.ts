...
function hasUsableProvenanceScheduleHash(raw: string | null): boolean {
  ...
  try {
    ...
    const scheduleHash = (parsed as { readonly scheduleHash?: unknown })
      .scheduleHash;
    return (
      typeof scheduleHash === "string" && /^[a-f0-9]{64}$/.test(scheduleHash)
    );
  } catch {
    return false;
  }
...
function main(): void {
  ...
  try {
    ...
    const validatedParameters = registry.validateParameters(
      request.strategyId,
      request.parameters,
    );
    if (!validatedParameters.ok)
      throw new Error(
        `app job 파라미터가 올바르지 않습니다: ${validatedParameters.error}`,
      );
    ...
    const compareArtifactProvenancePin = hasUsableProvenanceScheduleHash(
    ...
    const contextMatchesJob =
      ...
      artifact.context.feeModelVersion ===
        `${costProfile.id}@${costProfile.version}` &&
      artifact.context.slippageModelVersion ===
        `${slippageProfile.id}@${slippageProfile.version}` &&
