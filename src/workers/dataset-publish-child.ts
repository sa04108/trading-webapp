...
async function publish(input: { sourcePath: string; directory: string; version: number }): Promise<DatasetManifest> {
  ...
  try {
    ...
    try {
      ...
      const tables = snapshot.prepare("SELECT name FROM sqlite_master WHERE type = 'table'").all() as Array<{ name: string }>;
      if (tables.some(({ name }) => !allowed.has(name))) throw new Error('배포할 수 없는 운영 테이블이 계산 DB에 있습니다');
