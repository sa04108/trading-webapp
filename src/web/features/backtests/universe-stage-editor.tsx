...
const DIRECTION_OPTIONS: Record<
  UniverseCriterion,
  readonly [
 ...
 * 수 없는 화면이 된다.
 */
export function UniverseStageEditor({
  stages,
  onChange,
}: UniverseStageEditorProps) {
  ...
  const availableCriteria = ALL_CRITERIA.filter(
    (criterion) => !usedCriteria.has(criterion),
  );
  ...
  return (
    <div className="space-y-2">
      {stages.map((stage, index) => {
        const maxLimit =
          index === 0 ? FIRST_STAGE_LIMIT_MAX : stages[index - 1]!.limit;
          ...
          <div
            ...
            onDrop={(event) => {
              ...
              if (from !== null && from !== index)
                applyUpdate(moveStage(stages, from, index));
            }}
          >
            <span
              role="button"
            ...
            {stage.criterion === "DECLINE" ? (
              <div className="space-y-1">
                <Label htmlFor={`stage-lookback-${index}`}>
                  가격 변동 산정기간(거래일)
                </Label>
