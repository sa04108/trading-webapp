...
function reasonLabel(source: PreparationIssueSource, detail: string): string {
  ...
  if (source === "KRX_PRICE") {
    if (/거래일 일봉 부족/.test(detail))
      return "계산에 필요한 거래일 일봉 부족";
    if (/활성 기간의 KRX 일봉 .*누락/.test(detail))
      return "편입 기간의 일봉 누락";
  }
  if (source === "DART_FINANCIAL" || source === "DART_CORPORATE_ACTION") {
    ...
    if (detail.includes("응답 필드를 읽을 수 없습니다")) {
      const field = /응답 필드를 읽을 수 없습니다:\s*([^\s(/]+)/.exec(
        detail,
      )?.[1];
      ...
      return field && fieldLabels[field]
        ? `${fieldLabels[field]} 필드 누락`
        : "필수 응답 필드 누락";
    }
    if (detail.includes("분류할 수 없는 발행형태")) return "발행형태 분류 불가";
    if (detail.includes("정렬할 수 없는 자본변동"))
      return "KRX 상장주식수 변경일과 대응 불가";
    if (detail.includes("공시마다 다릅니다"))
      return "공시 간 자본변동 수치 불일치";
    if (detail.includes("주식분할 변동 수량"))
      return "발행주식수와 분할 수량 불일치";
    if (detail.includes("이벤트 직전 발행주식수"))
      return "보정에 필요한 직전 발행주식수 없음";
    if (detail.includes("보정 비율이 유효하지"))
      return "유효하지 않은 보정 비율";
    if (detail.includes("변동 수량을 읽을 수")) return "변동 수량 해석 불가";
    if (detail.includes("자본변동 일자를 읽을 수"))
      return "자본변동 일자 해석 불가";
    if (detail.includes("접수번호를 읽을 수")) return "접수번호 해석 불가";
    if (detail.includes("coverage")) return "필요 연도 데이터 확보 불가";
    if (detail.includes("재무 계정·연속 분기·신선도"))
      return "전략에 필요한 재무 이력 부족";
    if (detail.includes("재무 fact 없음"))
      return "편입 기간에 사용 가능한 재무 데이터 없음";
    if (detail.includes("PIT 재무 값 누락")) {
      return detail.includes("PER")
        ? "PER 계산에 필요한 재무 값 누락"
        : "ROE 계산에 필요한 재무 값 누락";
    }
    ...
    if (detail.includes("같은 보고서 안에서"))
      return "보고서 내 재무 값 불일치";
    if (detail.includes("직전 분기 누적값")) return "직전 분기 누적값 누락";
    if (detail.includes("직전 분기가 다른 사업연도"))
      return "연속 분기 사업연도 불일치";
    if (detail.includes("보고서 코드가 일치하지")) return "보고서 코드 불일치";
    if (detail.includes("행의 사업연도")) return "사업연도 불일치";
    if (detail.includes("응답 행이 모두 필터에서 제외"))
      return "사용 가능한 재무 응답 행 없음";
...
export function splitPreparationWarnings(warnings: readonly string[]): {
  ...
  for (const warning of warnings) {
    const excluded =
      /^(.*?) 정보를 온전히 확보할 수 없어 종목 ([A-Za-z0-9._-]+)을 매매 대상에서 제외했습니다 — ([\s\S]+)\.$/.exec(
        warning,
      );
      ...
      for (const cause of causes) {
        ...
        for (const example of financialExamples ?? [detail]) {
          ...
          issues.push({
            ...
            symbol: excluded[2]!,
            period:
              innerSeparator < 0 ? period : example.slice(0, innerSeparator),
            detail: actualDetail,
          });
        }
      }
    ...
    const notice = /^(?:.*\(([A-Za-z0-9._-]+)\)|([A-Za-z0-9._-]+)): (.+)$/.exec(
      warning,
    );
    ...
    if (
      notice &&
      detail &&
      /거래정지·무거래 기록|상장폐지됐습니다/.test(detail)
    ) {
      issues.push({
        source: "KRX_TRADING_STATUS",
        reason: detail.includes("상장폐지됐습니다")
          ? "상장폐지 이력"
          : "거래정지·무거래 이력",
        disposition: "NOTICE",
        symbol: (notice[1] ?? notice[2])!,
        period:
          /^\d{4}-\d{2}-\d{2}(?:~\d{4}-\d{2}-\d{2})?/.exec(detail)?.[0] ??
          "기간 정보 없음",
        detail,
      });
      continue;
...
export function groupPreparationIssues(
  issues: readonly PreparationIssue[],
): PreparationIssueGroup[] {
  ...
  for (const issue of issues) {
    ...
    details.set(JSON.stringify([issue.period, issue.detail]), {
      period: issue.period,
      detail: issue.detail,
    });
  }
