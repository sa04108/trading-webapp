...
import {
  groupPreparationIssues,
  PREPARATION_SOURCE_LABELS,
  splitPreparationWarnings,
...
function dispositionLabel(
  disposition: PreparationIssueGroup["disposition"],
): string {
...
export function PreparationIssueDetails({
  groups,
  mode,
}: {
  ...
  const rows: DetailRow[] = groups.flatMap((group) =>
    group.symbols.map((entry) => ({ group, entry })),
  );
  if (mode === "symbol")
    rows.sort((a, b) => a.entry.symbol.localeCompare(b.entry.symbol));
  return (
    <Table
      aria-label={
        mode === "reason" ? "확인사항 기본보기" : "확인사항 종목별보기"
      }
    ...
    >
      <TableHeader>
        <TableRow>
          {mode === "symbol" ? (
            <TableHead scope="col">종목코드</TableHead>
          ) : null}
      ...
      <TableBody>
        {rows.map(({ group, entry }, index) => {
          ...
          const sourceSpan = rowSpan(rows, index, (row) =>
            mode === "symbol"
              ? `${row.entry.symbol}:${row.group.source}`
              : row.group.source,
          );
          const reasonSpan =
            mode === "reason"
              ? rowSpan(rows, index, (row) => row.group.key)
              : 1;
          return (
            <TableRow key={`${group.key}:${entry.symbol}`}>
              {mode === "symbol" && symbolSpan > 0 ? (
                <TableCell rowSpan={symbolSpan} className="align-top font-mono">
                  {entry.symbol}
                </TableCell>
              ) : null}
              {sourceSpan > 0 ? (
              ...
              <TableCell className="min-w-32 align-top whitespace-pre-line">
                {[
                  ...new Set(entry.details.map((detail) => detail.period)),
                ].join("\n")}
              </TableCell>
...
export function PreparationIssuesCard({
  warnings,
  className,
  result = false,
}: {
  ...
  const uniqueSymbols = new Set(parsed.issues.map((issue) => issue.symbol))
    .size;
      ...
      >
        ...
        <CardContent className="space-y-3 text-sm">
              ...
              <TableBody>
                {groups.map((group) => (
                  <TableRow key={group.key}>
                    ...
                    <TableCell className="min-w-36 whitespace-normal wrap-anywhere">
                      {group.reason}
                    </TableCell>
                    <TableCell>{dispositionLabel(group.disposition)}</TableCell>
                    <TableCell className="text-right tabular-nums">
                      {group.symbols.length.toLocaleString("ko-KR")}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
          ...
          <p className="text-xs text-muted-foreground">
            중복 제외 {uniqueSymbols.toLocaleString("ko-KR")}종목 · 같은 종목이
            서로 다른 사유에 포함될 수 있습니다.
          </p>
          {parsed.otherWarnings.length > 0 ? (
            <p className="text-xs text-muted-foreground">
              기타 확인사항 {parsed.otherWarnings.length}건은 자세히 보기에서
              확인할 수 있습니다.
            </p>
          ) : null}
          <p className="text-xs text-muted-foreground">
            {result
              ? "이 백테스트를 준비할 당시 확인된 내용입니다."
              : "확인사항을 검토한 뒤 그대로 진행할 수 있습니다."}
          </p>
        </CardContent>
        <DialogContent className="flex max-h-[90dvh] min-w-0 flex-col sm:max-w-5xl">
          <DialogHeader className="pr-8">
            ...
            <DialogDescription>
              해당 미리보기에서 확인된 사유와 모든 종목코드입니다. 중복 제외{" "}
              {uniqueSymbols.toLocaleString("ko-KR")}종목.
            </DialogDescription>
          </DialogHeader>
          <Tabs
            value={mode}
            onValueChange={(value) => setMode(value as DetailMode)}
                ...
                {parsed.otherWarnings.length > 0 ? (
                  <div className="space-y-2 border-t p-3">
                    ...
                    <ul className="list-disc space-y-2 pl-4 wrap-anywhere">
                      {parsed.otherWarnings.map((warning, index) => (
