...
type RepeatedWarningKind =
  "cash" | "nonTrading" | "outsideUniverse" | "retiredCode";
...
function stripTargetSymbols(warning: string): string {
  if (
    !warning.startsWith("동시 보유 종목 상한(") &&
    !warning.startsWith("유동성 체결 한도:")
  ) {
    return warning;
  }
  return warning.replace(
    new RegExp(
      `— 대상 (\\d+)종목: ${SYMBOL}(?:, ${SYMBOL})*(?: 외 \\d+종목)?\\.`,
    ),
    "— 대상 $1종목.",
  );
}
...
function summarizeDatasetDrift(warning: string): string {
  const match = /^제출 이후 데이터가 변경된 종목이 있습니다: (.+) — (.+)$/.exec(
    warning,
  );
...
function summarizeOne(warning: string): string {
  return summarizeDatasetDrift(
    stripDelimitedSymbols(stripTargetSymbols(warning)),
  );
}

/** 실행 결과 경고에서 종목 코드를 제거하고 반복 경고를 건수로 접는다. */
export function summarizeExecutionWarnings(
  warnings: readonly string[],
): string[] {
  const counts = new Map<
    RepeatedWarningKind,
    { events: number; symbols: Set<string> }
  >();
...
export function isExecutionLimitation(warning: string): boolean {
  return (
    limitationPrefixes.some((prefix) => warning.startsWith(prefix)) ||
