      ...
      <CardContent className="space-y-3 text-sm">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span>
              전체 진행률 <span className="text-muted-foreground">(예상)</span>
            </span>
        ...
        {job.status === "WAITING_DAILY_QUOTA" ? (
          <Alert role="alert">
            <AlertDescription>
              ...
              {formatPreparationResumeTime(job.nextResumeAtMs)}에 자동으로
              이어서 준비합니다.
            </AlertDescription>
          </Alert>
        ) : null}
        ...
        job.status === "RUNNING" ||
        job.status === "WAITING_DAILY_QUOTA" ||
        job.status === "WAITING_DATA" ? (
          <p className="text-xs text-muted-foreground">
            백그라운드 준비 작업은 화면을 나가거나 브라우저를 닫아도 계속됩니다.
            나중에 같은 조건으로 미리보기를 누르면 진행 상황을 다시 볼 수
            있으며, 완료·실패 결과는 알림에서 확인할 수 있습니다.
          </p>
        ) : null}
