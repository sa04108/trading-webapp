...
export function BenchmarkPanel() {
  ...
  const query = useQuery({
    ...
    refetchInterval: (current) =>
      current.state.data?.backfill.state === "RUNNING" ? 1_000 : false,
  });
  ...
  >({
    ...
    onError: (error: unknown) =>
      toast.error(
        error instanceof Error ? error.message : "수집 시작에 실패했습니다",
      ),
  });

  const points = query.data?.points ?? [];
  const backfillStatus =
    query.isFetching && backfill.data?.state === "RUNNING"
      ? backfill.data
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle className="text-base">벤치마크</CardTitle>
        </CardHeader>
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="space-y-1">
            ...
            <Select
              value={benchmarkId}
              onValueChange={(value) => setBenchmarkId(value as BenchmarkId)}
            >
              <SelectTrigger id="benchmark-id" className="w-full">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      {backfillStatus?.state === "RUNNING" ? (
        <Alert>
          <AlertDescription>
            {backfillStatus.benchmarkId === null
              ? "벤치마크"
              : BENCHMARK_NAMES[backfillStatus.benchmarkId]}{" "}
      ...
      {query.data && !query.data.covered ? (
        <Alert>
          <AlertDescription>벤치마크 데이터가 부족합니다.</AlertDescription>
        </Alert>
      ) : null}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            ...
            {returnPct === null
              ? ""
              : ` · 기간 수익률 ${returnPct >= 0 ? "+" : ""}${returnPct.toFixed(2)}%`}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {query.isLoading ? (
          ...
          {points.length > 0 ? (
            <div
              ...
              role="img"
              aria-label={`${BENCHMARK_NAMES[benchmarkId]} 종가 차트`}
            >
              <ResponsiveContainer>
                <LineChart
                  data={points}
