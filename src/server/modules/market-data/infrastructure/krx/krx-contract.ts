import { z } from "zod";
import { KrxContractError } from "../../../../../runtime/modules/market-data/application/ports.js";
import { basDdToIso } from "../../../../../runtime/modules/market-data/domain/kst-date.js";
import type {
  KrxDailyTradeRow,
  KrxIssueBaseInfoRow,
} from "../../../../../runtime/modules/market-data/domain/krx-universe-types.js";

export { KRX_CONTRACT_VERSION } from "../../../../../runtime/modules/market-data/domain/krx-universe-types.js";
...
export function parseKrxEnvelope(
  payload: unknown,
...
export function parseNullableInt64(
  ...
  if (!/^\d+$/.test(trimmed)) {
    throw new KrxContractError(
      `${field} 가 정수 형식이 아닙니다: ${trimmed.slice(0, 30)}`,
    );
  }
...
function parseNullableDecimalString(
  raw: string | null | undefined,
  field: string,
): string | null {
  const trimmed = (raw ?? "").trim().replaceAll(",", "");
...
export function parseBaseInfoRows(
  rows: readonly Record<string, unknown>[],
): KrxIssueBaseInfoRow[] {
  return rows.map((rawRow) => {
    const parsed = baseInfoRowSchema.safeParse(rawRow);
    if (!parsed.success)
      throw contractErrorFromZod("기본정보 행", parsed.error);
...
function parseNullableIntNumber(
  raw: string | null | undefined,
  field: string,
): number | null {
...
export function parseDailyRows(
  ...
  return rows.map((rawRow) => {
    ...
    const tradingValue = parseNullableDecimalString(
      row.ACC_TRDVAL,
      "ACC_TRDVAL",
    );
    return {
...
export function parseIndexClose(
  ...
  for (const rawRow of rows) {
    ...
    if (raw === "" || !Number.isFinite(close) || close <= 0) {
      throw new KrxContractError(
        `CLSPRC_IDX 가 양수 형식이 아닙니다: ${raw.slice(0, 30)}`,
      );
    }
    return close;
  }
