...
export function registerSymbolMasterRoutes(
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      if (!parsed.success) {
        return reply
          .code(400)
          .send({ error: "date(YYYY-MM-DD) 쿼리가 필요합니다" });
      }
  ...
  app.get("/symbol-master/coverage", { preHandler: requireAuth }, async () => {
    ...
    const lastSyncedAtMs =
      ranges.length === 0
        ? null
        : Math.max(...ranges.map((range) => range.syncedAtMs));
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      if (!parsed.success) {
        return reply
          .code(400)
          .send({ error: "from/to(YYYY-MM-DD) 쿼리가 필요합니다" });
      }
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!parsed.success) {
        return reply
          .code(400)
          .send({ error: "date(YYYY-MM-DD) 필드가 필요합니다" });
      }
      try {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!parsed.success) {
        return reply
          .code(400)
          .send({ error: "fromDate(YYYY-MM-DD) 필드가 필요합니다" });
      }
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      if (!parsed.success || parsed.data.from > parsed.data.to) {
        return reply
          .code(400)
          .send({ error: "benchmarkId와 올바른 from/to 날짜가 필요합니다" });
      }
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!parsed.success || parsed.data.from > parsed.data.to) {
        return reply
          .code(400)
          .send({ error: "올바른 from/to 날짜가 필요합니다" });
      }
      return reply
        .code(202)
        .send(
          deps.benchmarks.startBackfill(
            parsed.data.benchmarkId,
            parsed.data.from,
            parsed.data.to,
          ),
        );
