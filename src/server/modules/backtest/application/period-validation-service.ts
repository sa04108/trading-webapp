...
import {
  backtestValidations,
  backtestValidationTrials,
} from "../../../shared/db/backtest-validation-schema.js";
...
export interface PeriodValidationDeps {
  ...
  ): Promise<(() => BacktestJobRow | null) | null>;
...
export class PeriodValidationService {
  ...
  plan(sourceJobId: string, rawConfig: unknown) {
    ...
    if (!sourceJob || sourceJob.status !== "COMPLETED")
      throw new Error("완료된 백테스트에서 검증을 시작할 수 있습니다.");
    const source = backtestRequestSchema.parse(
      JSON.parse(sourceJob.requestJson),
    );
    ...
    if (
      !run ||
      run.strategyVersion !== strategy.version ||
      run.strategySourceHash !== strategySourceHash(strategy)
    ) {
      throw new Error(
        "원본과 현재 전략 버전이 다릅니다. 현재 전략으로 백테스트를 다시 실행하세요.",
      );
    }
    if (
      run.executionVersion !== readRuntimeVersions().executionVersion ||
      run.engineVersion !== ENGINE_VERSION
    ) {
      throw new Error(
        "원본과 현재 실행 버전이 다릅니다. 현재 실행 버전으로 백테스트를 다시 실행하세요.",
      );
    }
    if (config.mode !== "HOLDOUT") {
      const schema = this.deps.strategies.getParameterJsonSchema(
        source.strategyId,
      );
      ...
      for (const axis of config.optimization.axes) {
        if (
          !["number", "integer"].includes(properties?.[axis.key]?.type ?? "")
        ) {
    ...
    for (const parameters of [...plan.candidates, source.parameters]) {
      const validated = this.deps.strategies.validateParameters(
        source.strategyId,
        parameters,
      );
      ...
      for (const fold of plan.folds) {
        for (const period of [fold.train, fold.test]) {
          const errors = this.deps.validateRequest({
            ...source,
            parameters,
            period,
          });
  ...
  create(sourceJobId: string, rawConfig: unknown): PeriodValidationDto {
    const { source, config, plan, strategy } = this.plan(
      sourceJobId,
      rawConfig,
    );
    ...
    this.deps.database.sqlite
      .transaction(() => {
        if (this.deps.queue.getJob(sourceJobId)?.status !== "COMPLETED")
          throw new Error("원본 백테스트 상태가 변경됐습니다.");
        const active = this.deps.database.db
          .select({ id: backtestValidations.id })
          .from(backtestValidations)
          .where(inArray(backtestValidations.status, ["ACTIVE", "CANCELLING"]))
          .all();
        if (active.length >= 5)
          throw new Error("진행 중인 검증 실험은 최대 5개입니다.");
        this.deps.database.db
          .insert(backtestValidations)
          .values({
            id,
            sourceJobId,
        ...
        for (const fold of plan.folds) {
          for (const [candidate, parameters] of plan.candidates.entries()) {
            this.deps.database.db
              .insert(backtestValidationTrials)
              .values({
                id: newId("vtr"),
                validationId: id,
                fold: fold.ordinal,
  ...
  list(sourceJobId: string): PeriodValidationDto[] {
    return this.deps.database.db
      .select({ id: backtestValidations.id })
      .from(backtestValidations)
      .where(eq(backtestValidations.sourceJobId, sourceJobId))
      .orderBy(asc(backtestValidations.createdAtMs))
      .all()
      .map(({ id }) => this.get(id)!);
  }
  ...
  get(id: string): PeriodValidationDto | null {
    ...
    return {
      id,
      sourceJobId: experiment.sourceJobId,
      status: experiment.status as ValidationStatus,
      config: JSON.parse(experiment.configJson) as PeriodValidationConfig,
      plan,
      initialCash: source.capital.initialCash,
      createdAtMs: experiment.createdAtMs,
      error: experiment.error,
      trials: this.trials(id).map((trial) => {
        const request = trial.requestJson
          ? (JSON.parse(trial.requestJson) as BacktestRequest)
          : null;
        const job = trial.jobId ? this.deps.queue.getJob(trial.jobId) : null;
        const prep = trial.preparationJobId
          ? this.deps.preparation.get(trial.preparationJobId)
          : null;
        return {
          id: trial.id,
          fold: trial.fold,
          role: trial.role as ValidationRole,
          candidate: trial.candidate,
          period: request?.period ?? plan.folds[trial.fold]!.test,
          ...
          status:
            ...
            (experiment.status === "ACTIVE"
              ? prep && prep.status !== "COMPLETED"
                ? `PREPARATION_${prep.status}`
                : "PENDING"
              : experiment.status === "COMPLETED"
                ? "MISSING"
                : "SKIPPED"),
          progress: job?.totalBars
            ? Math.min(100, ((job.progressBars ?? 0) / job.totalBars) * 100)
            : null,
          error: job?.error ?? prep?.error ?? null,
          metrics: job?.status === "COMPLETED" ? this.metrics(job.id) : null,
          benchmarkReturnPct:
            job?.status === "COMPLETED"
              ? (this.deps.results.getBenchmark(job.id)?.totalReturnPct ?? null)
              : null,
        };
      }),
    };
  ...
  referencesJob(jobId: string): boolean {
    return (
      this.deps.database.db
        .select({ id: backtestValidationTrials.id })
        .from(backtestValidationTrials)
        .where(eq(backtestValidationTrials.jobId, jobId))
        .get() !== undefined
    );
  }
  ...
  cancel(id: string): PeriodValidationDto | null {
    this.deps.database.db
      .update(backtestValidations)
      .set({ status: "CANCELLING" })
      .where(
        and(
          eq(backtestValidations.id, id),
          eq(backtestValidations.status, "ACTIVE"),
        ),
      )
      .run();
    return this.get(id);
  ...
  delete(id: string): boolean {
    return this.deps.database.sqlite
      .transaction(() => {
        const row = this.row(id);
        if (!row) return false;
        if (row.status === "ACTIVE" || row.status === "CANCELLING")
          throw new Error("검증 실험을 취소한 뒤 삭제하세요.");
        ...
        `,
              ...
              .get(trial.jobId, trial.jobId)
          )
            throw new Error("하위 백테스트에서 만든 실험을 먼저 삭제하세요.");
        }
        if (
          trials.some(
            (trial) =>
              trial.jobId &&
              !this.deps.queue.isTerminal(
                this.deps.queue.getJob(trial.jobId)?.status ?? "FAILED",
              ),
          )
        ) {
        ...
        this.deps.database.db
          .delete(backtestValidations)
          .where(eq(backtestValidations.id, id))
          .run();
        for (const trial of trials)
          if (trial.jobId) this.deps.queue.deleteJob(trial.jobId);
  ...
  pump(): Promise<void> {
    ...
    this.pumping = this.tick().finally(() => {
      this.pumping = null;
    });
    return this.pumping;
  }
  ...
  private trials(id: string): Trial[] {
    return this.deps.database.db
      ...
      .orderBy(
        asc(backtestValidationTrials.fold),
        asc(backtestValidationTrials.id),
      )
      .all();
  }
  ...
  private metrics(jobId: string): ValidationMetrics | null {
    return this.deps.results.getMetrics(
      jobId,
    ) as unknown as ValidationMetrics | null;
  }
  ...
  private samePhase(experiment: Experiment): boolean {
    ...
    return (
      ...
      current.fold === experiment.fold &&
      current.phase === experiment.phase
    );
  }
  ...
  private finish(
    ...
    const changed = this.deps.database.db
      .update(backtestValidations)
      .set({ status })
      .where(
        and(
          eq(backtestValidations.id, id),
          inArray(backtestValidations.status, ["ACTIVE", "CANCELLING"]),
        ),
      )
      .run();
  ...
  private assertVersion(experiment: Experiment): void {
    ...
    if (
      !strategy ||
      strategy.version !== experiment.strategyVersion ||
      strategySourceHash(strategy) !== experiment.strategySourceHash ||
      ENGINE_VERSION !== experiment.engineVersion ||
      executionVersion !== experiment.executionVersion ||
      validationVersion !== experiment.validationVersion
    ) {
      throw new Error(
        "실험 도중 전략 또는 실행 버전이 변경됐습니다. 새 실험을 생성하세요.",
      );
    }
  }
  ...
  private assertRevision(experiment: Experiment): void {
    if (experiment.dataRevision !== this.cache.revision()) {
      throw new Error(
        "같은 회차를 비교하는 도중 시장·재무 데이터가 변경됐습니다. 서로 다른 데이터의 성과를 비교하지 않도록 실험을 중단했습니다.",
      );
    }
  }

  private async tick(): Promise<void> {
    const experiments = this.deps.database.db
      .select()
      .from(backtestValidations)
      .where(inArray(backtestValidations.status, ["ACTIVE", "CANCELLING"]))
      .orderBy(asc(backtestValidations.createdAtMs))
    ...
    for (const experiment of experiments.filter(
      (row) => row.status === "CANCELLING",
    ))
      this.finishCancellation(experiment);
    ...
    try {
      ...
      const group = this.trials(experiment.id).filter(
        (trial) =>
          trial.fold === experiment.fold &&
      ...
      this.deps.database.db
        ...
        .where(eq(backtestValidations.id, experiment.id))
        .run();
  ...
  private finishCancellation(experiment: Experiment): void {
    ...
    for (const trial of this.trials(experiment.id)) {
      ...
      if (!prep || ["COMPLETED", "FAILED", "CANCELLED"].includes(prep.status))
        continue;
      const shared = this.deps.database.sqlite
        .prepare(
          `
    ...
    if (!pending)
      this.finish(experiment.id, experiment.error ? "FAILED" : "CANCELLED");
  }

  private attachPreparation(trial: Trial, preparationJobId: string): void {
    this.deps.database.db
      .update(backtestValidationTrials)
      .set({ preparationJobId })
      .where(eq(backtestValidationTrials.id, trial.id))
      .run();
    this.deps.database.db
      .update(backtestPreparationJobs)
      .set({ lifecycleManaged: true })
      .where(eq(backtestPreparationJobs.id, preparationJobId))
      .run();
    this.deps.collectPreparations();
  }
    ...
    for (const trial of group) {
      ...
      if (trial.preparationJobId) {
        ...
        if (job.status === "FAILED" || job.status === "CANCELLED")
          throw new Error(job.error ?? "데이터 준비가 종료됐습니다.");
        if (job.status !== "COMPLETED") return;
        if (
          this.deps.preparation.getFreshPreviewDetails(
            request,
            trial.preparationJobId,
          )
        )
          continue;
      }
      ...
      this.deps.database.sqlite
        .transaction(() => {
          ...
          if (
            !currentTrial ||
            currentTrial.preparationJobId !== trial.preparationJobId
          )
            return;
          if (
            currentTrial.preparationJobId &&
            this.cache.isFresh(currentTrial.preparationJobId)
          )
            return;
      ...
      .transaction(() => {
        ...
        if (
          !group.every(
            (trial) =>
              trial.preparationJobId &&
              this.cache.isFresh(trial.preparationJobId),
          )
        )
          return;
        this.deps.database.db
          .update(backtestValidations)
          .set({ phase: training ? "TRAIN" : "OOS", dataRevision: revision })
          .where(eq(backtestValidations.id, experiment.id))
          .run();
      })
      .immediate();
  ...
  private async runGroup(
    ...
    for (const trial of group) {
      ...
      if (
        !job ||
        (this.deps.queue.isTerminal(job.status) && job.status !== "COMPLETED")
      )
        throw new Error(
          job?.error ?? "하위 백테스트가 정상 완료되지 않았습니다.",
        );
      if (job.status === "COMPLETED") {
        ...
        if (
          !run ||
          run.strategySourceHash !== experiment.strategySourceHash ||
          run.engineVersion !== experiment.engineVersion ||
          run.executionVersion !== experiment.executionVersion
        )
          throw new Error("하위 백테스트의 실행 버전이 실험과 다릅니다.");
        if (!metrics || !Number.isFinite(metrics.totalReturnPct))
          throw new Error("하위 백테스트의 성과 지표가 없습니다.");
      }
    }
    const pending = group.find((trial) => !trial.jobId);
    if (pending) {
      const request = JSON.parse(pending.requestJson!) as BacktestRequest;
      const preview = this.deps.preparation.getFreshPreviewDetails(
        request,
        pending.preparationJobId!,
      )?.preview;
      if (!preview)
        throw new Error("고정한 준비 결과가 더 이상 유효하지 않습니다.");
      ...
      this.deps.database.sqlite
        .transaction(() => {
          ...
          const currentTrial = this.deps.database.db
            .select()
            .from(backtestValidationTrials)
            .where(eq(backtestValidationTrials.id, pending.id))
            .get();
    ...
    if (
      !group.every(
        (trial) => this.deps.queue.getJob(trial.jobId!)?.status === "COMPLETED",
      )
    )
      return;
    ...
    if (training) {
      const candidate =
        config.mode === "HOLDOUT"
          ...
          : selectValidationCandidate(
              group.map((trial) => ({
                candidate: trial.candidate!,
                metrics: this.metrics(trial.jobId!)!,
              })),
              config.optimization,
            );
      if (candidate === null)
        throw new Error(
          `${experiment.fold + 1}회차 IS에서 선택 조건을 만족하는 후보가 없습니다. 이 회차를 제외하지 않고 실험을 중단했습니다.`,
        );
      const source = JSON.parse(experiment.requestJson) as BacktestRequest;
      this.deps.database.sqlite
        .transaction(() => {
          ...
          this.deps.database.db
            ...
            .set({
              ...
              requestJson: JSON.stringify({
                ...source,
                parameters: plan.candidates[candidate],
                period: plan.folds[experiment.fold]!.test,
              }),
            })
            .where(
              and(
                eq(backtestValidationTrials.validationId, experiment.id),
                eq(backtestValidationTrials.fold, experiment.fold),
                eq(backtestValidationTrials.role, "OOS"),
              ),
            )
            .run();
          this.deps.database.db
            .update(backtestValidations)
            .set({ phase: "PREPARING_OOS", dataRevision: null })
            .where(eq(backtestValidations.id, experiment.id))
            .run();
        })
        .immediate();
      ...
      this.deps.database.sqlite
        .transaction(() => {
          ...
          this.assertVersion(experiment);
          this.assertRevision(experiment);
          if (experiment.fold + 1 === plan.folds.length)
            this.finish(experiment.id, "COMPLETED");
          else
            this.deps.database.db
              .update(backtestValidations)
              .set({
                fold: experiment.fold + 1,
                phase: "PREPARING_TRAIN",
                dataRevision: null,
              })
              .where(eq(backtestValidations.id, experiment.id))
              .run();
