...
} from "../../../shared/db/backtest-batch-schema.js";
...
export class SeedCloneBatchService {
  ...
  create(
    sourceJobId: string,
    count: number,
    snapshot: SeedCloneBatchSnapshot,
    ...
    this.database.sqlite
      .transaction(() => {
        ...
        const preparationJobId =
          ...
          this.queue.getJob(sourceJobId)?.preparationJobId ??
          null;
        if (preparationJobId)
          new PreparationReferenceService(this.database).requirePreparation(
            preparationJobId,
            true,
          );
        tx.insert(backtestCloneBatches)
          .values({
            id: batchId,
            preparationJobId,
            sourceJobId,
          ...
          .run();
        tx.insert(backtestCloneBatchItems)
          .values(
  ...
  pump(): void {
    let available =
      this.maxQueuedBacktests - this.queue.countByStatus(["QUEUED"]);
          ...
          .transaction(() => {
            ...
            const job = this.queue.enqueue(
              ...
              snapshot.benchmark,
              {
                ...
                cloneSourceJobId: batch.sourceJobId,
                preparationJobId: batch.preparationJobId,
              },
            );
            this.database.db
              .update(backtestCloneBatchItems)
              .set({
                state: "DISPATCHED",
                jobId: job.id,
              })
  ...
  onJobStatusChanged(): void {
    ...
    for (const batch of this.database.db
      ...
      if (allCancelledOrTerminal)
        this.markTerminal(batch.id, "CANCELLED", ["CANCELLING"]);
    }
  }
  ...
  cancel(batchId: string): SeedCloneBatchDetail | null {
    ...
    this.database.db.transaction((tx) => {
      tx.update(backtestCloneBatches)
        .set({
          status: "CANCELLING",
          completedAtMs: null,
        })
      ...
      for (const { item } of existing.items) {
        ...
        tx.update(backtestCloneBatchItems)
          .set({ state: "CANCELLED" })
          .where(eq(backtestCloneBatchItems.id, item.id))
          .run();
      }
  ...
  deleteSourceJob(sourceJobId: string): SeedCloneDeleteResult {
    return this.database.sqlite
      .transaction(() => {
        ...
        this.database.db
          .delete(backtestJobs)
          .where(eq(backtestJobs.id, sourceJobId))
          .run();
        new PreparationReferenceService(this.database).collect();
        return "DELETED";
      })
   ...
   */
  private collectDeletionPlan(
    rootBatchIds: readonly string[],
  ): SeedCloneDeletionPlan | null {
  ...
  private executeDeletionPlan(plan: SeedCloneDeletionPlan): void {
    if (plan.jobIds.length > 0) {
      this.database.db
        .delete(backtestJobs)
        .where(inArray(backtestJobs.id, [...plan.jobIds]))
        .run();
    }
...
function parseSnapshot(batch: SeedCloneBatchRow): SeedCloneBatchSnapshot {
  ...
  const schedule = JSON.parse(
    batch.universeScheduleJson,
  ) as LegacyUniverseScheduleEntry[];
  const universeEntries =
    batch.universeJson === null ? [] : JSON.parse(batch.universeJson);
  const provenancePin = JSON.parse(
    batch.provenancePinJson ?? "null",
  ) as ProvenancePin | null;
  const benchmark = benchmarkPinSchema.parse(
    JSON.parse(batch.benchmarkJson ?? "null"),
  );
  const warnings =
    batch.submitWarningsJson === null
      ? []
      : (JSON.parse(batch.submitWarningsJson) as string[]);
  if (!Array.isArray(schedule) || schedule.length === 0)
    throw new Error("저장된 유니버스 일정이 없습니다.");
