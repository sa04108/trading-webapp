...
function isMalformedArtifactDatabase(error: unknown): boolean {
  ...
  return (
    (typeof code === "string" &&
      ["SQLITE_NOTADB", "SQLITE_CORRUPT", "SQLITE_ERROR"].some(
        (base) => code === base || code.startsWith(`${base}_`),
      )) ||
...
export class SqliteBacktestResultArtifactImporter implements BacktestResultArtifactImporter {
  ...
  validate(
    ...
    if (
      !stat.isFile() ||
      stat.size <= 0 ||
      stat.size > MAX_BACKTEST_RESULT_ARTIFACT_BYTES
    ) {
      throw new InvalidBacktestResultArtifactError(
        "결과 artifact 파일 크기가 허용 범위를 벗어납니다",
      );
    }
    ...
    try {
      ...
      if (schemaVersion !== BACKTEST_RESULT_ARTIFACT_SCHEMA_VERSION) {
        throw new InvalidBacktestResultArtifactError(
          `지원하지 않는 결과 schema: ${schemaVersion}`,
        );
      }
      ...
      if (
        JSON.stringify(schemaObjects) !== JSON.stringify(expectedSchemaObjects)
      ) {
        throw new InvalidBacktestResultArtifactError(
          "결과 artifact schema에 허용되지 않은 객체가 있습니다",
        );
      }
      ...
      const manifest = sqlite
        ...
        .get() as
        | { schemaVersion: number; contextJson: string; summaryJson: string }
        | undefined;
      if (manifest === undefined || manifest.schemaVersion !== schemaVersion) {
        throw new InvalidBacktestResultArtifactError(
          "결과 manifest가 없거나 schema가 일치하지 않습니다",
        );
      }
      ...
      if (context.jobId !== expectedJobId) {
        throw new InvalidBacktestResultArtifactError(
          "artifact jobId가 lease jobId와 다릅니다",
        );
      }
      const counts = sqlite
        .prepare(
      ...
      if (
        counts.equityCount !== counts.drawdownCount ||
        counts.tradeCount !== summary.metrics.tradeCount
      ) {
        throw new InvalidBacktestResultArtifactError(
          "결과 행 수와 summary 지표가 일치하지 않습니다",
        );
      }
      const seriesMismatch = sqlite
        .prepare(
      ...
      const contiguousSeries =
        counts.equityCount === 0
  ...
  write(artifact: ValidatedBacktestResultArtifact): void {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        throw new InvalidBacktestResultArtifactError(
          "결과 artifact 행 값이 올바르지 않습니다",
          {
            cause: error,
          },
        );
      }
      throw new BacktestResultPersistenceError(
        "중앙 결과 저장소에 artifact를 쓸 수 없습니다",
        {
          cause: error,
        },
      );
