...
import {
  ...
  type PeriodValidationDeps,
} from "../application/period-validation-service.js";
...
export function registerPeriodValidationRoutes(
  ...
  const pump = (): void => {
    void service
      .pump()
      .catch((error: unknown) =>
        app.log.error({ err: error }, "기간 검증 작업 진행 실패"),
      );
  };
  ...
  app.post(
    "/backtests/:id/validations/preview",
  ...
  app.get(
    ...
    (request) => {
      return {
        experiments: service.list((request.params as { id: string }).id),
      };
    },
  );
  app.get(
    "/backtest-validations/:id",
    { preHandler: requireAuth },
    (request, reply) => {
  ...
  app.post(
    "/backtest-validations/:id/cancel",
    { preHandler: requireAuth },
    (request, reply) => {
      const experiment = service.cancel((request.params as { id: string }).id);
      if (!experiment)
        return reply.code(404).send({ error: "검증 실험을 찾을 수 없습니다." });
      pump();
      return { experiment };
    },
  );
