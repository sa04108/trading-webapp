...
import type {
  BacktestJobRow,
  JobQueue,
} from "../modules/backtest/application/job-queue.js";
import type {
...
const TERMINAL_NOTIFICATIONS: Partial<
  Record<string, { title: string; severity: "info" | "error" }>
> = {
...
function parseSubmitWarnings(
  ...
  try {
    ...
    logger.warn(
      {
        module: "notification",
        event: "notify.warnings.parse_failed",
        jobId,
        err: error,
      },
...
export function createBacktestNotificationListener(deps: {
  ...
  return (event) => {
    ...
    try {
      const job: BacktestJobRow | null | undefined = deps.queue.getJob(
        event.jobId,
      );
      if (!job) return;
      ...
      const warnings = parseSubmitWarnings(
        job.submitWarningsJson,
        deps.logger,
        job.id,
      );
      deps.notify({
...
export function createSeedCloneBatchNotificationListener(deps: {
  ...
  return (event) => {
    try {
      ...
      const label =
        deps.strategyName(detail.batch.strategyId) ?? detail.batch.strategyId;

      deps.notify({
        type: "backtest",
        severity:
          event.status === "FAILED" || failed + interrupted > 0
            ? "error"
            : "info",
