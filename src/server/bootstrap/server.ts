...
} from "../modules/agents/presentation/agent-routes.js";
...
export async function buildServer(
  ...
  await app.register(fastifyWebsocket, {
    options: { maxPayload: AGENT_MAX_MESSAGE_BYTES },
  });
  ...
  await app.register(
    async (api) => {
      ...
      registerSymbolRoutes(api, container.symbolInfoService, requireAuth);
          ...
          onValidationFinished: (experiment) => {
            try {
              container.notificationService.create({
                type: "backtest",
                severity: experiment.status === "FAILED" ? "error" : "info",
                title:
                ...
                body:
                  experiment.error ??
                  `${experiment.plan.folds.length}개 구간 · ${experiment.plan.totalRuns}회 실행 계획`,
  ...
  await app.register(
    async (api) => {
      registerAgentControlRoutes(
        api,
        container.agentCoordinator,
        container.remoteResultUploadManager,
      );
    },
    { prefix: "/api/agents" },
  );
  ...
  if (publicDir) {
    ...
    app.setNotFoundHandler((request, reply) => {
      ...
      return reply
        .code(404)
        .send({ error: "요청한 리소스를 찾을 수 없습니다" });
    });
  } else {
    app.setNotFoundHandler((_request, reply) =>
      reply.code(404).send({ error: "요청한 리소스를 찾을 수 없습니다" }),
    );
  }
