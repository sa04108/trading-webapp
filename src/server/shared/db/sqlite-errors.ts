...
function hasCode(error: unknown, codes: readonly string[]): boolean {
  ...
  for (
    ...
    if (
      ...
      codes.some(
        (code) => actualCode === code || actualCode.startsWith(`${code}_`),
      )
    )
      return true;
    current = value.cause;
...
export function isPersistenceUnavailableError(error: unknown): boolean {
  if (
    hasCode(error, UNAVAILABLE_STORAGE_CODES) ||
  ...
  for (
    ...
    if (
      typeof value.message === "string" &&
      /database or disk is full|database is read-only|readonly database/i.test(
        value.message,
      )
    )
      return true;
