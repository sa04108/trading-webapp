...
interface BackupManifest { version: 1; operationsSha256: string; dataSha256: string | null }
...
export async function backupDatabase(databasePath: string, snapshotPath: string): Promise<void> {
  ...
  let split: boolean;
  try { split = tableExists(source, 'operational_database_state'); } finally { source.close(); }
  ...
  try {
    ...
    if (split) await copyConsistent(dataDatabasePath(databasePath), `${snapshotPath}.data`);
    const manifest: BackupManifest = { version: 1, operationsSha256: await checksum(snapshotPath), dataSha256: split ? await checksum(`${snapshotPath}.data`) : null };
...
export async function restoreDatabase(databasePath: string, snapshotPath: string): Promise<void> {
  ...
  if (manifest.version !== 1 || !/^[a-f0-9]{64}$/.test(manifest.operationsSha256)
    || manifest.dataSha256 !== null && !/^[a-f0-9]{64}$/.test(manifest.dataSha256)) throw new Error('DB 백업 명세가 올바르지 않습니다');
  if (await checksum(snapshotPath) !== manifest.operationsSha256
    || manifest.dataSha256 !== null && await checksum(`${snapshotPath}.data`) !== manifest.dataSha256) throw new Error('DB 백업 파일 해시가 다릅니다');
  ...
  if (manifest.dataSha256 === null) fs.rmSync(dataDatabasePath(databasePath), { force: true });
  else if (fs.existsSync(`${journal.staged}.data`)) fs.renameSync(`${journal.staged}.data`, dataDatabasePath(databasePath));
  ...
  if (await checksum(databasePath) !== manifest.operationsSha256
    || manifest.dataSha256 !== null && await checksum(dataDatabasePath(databasePath)) !== manifest.dataSha256) throw new Error('복원된 DB 파일이 백업과 다릅니다');
  const splitJournal = `${databasePath}.split-migration.json`;
  if (fs.existsSync(splitJournal)) fs.renameSync(splitJournal, `${splitJournal}.rolled-back-${randomUUID()}`);
