...
export function calculateResources(
  ...
  const possible = Math.min(
    cpuSlots,
    running + Math.floor(availableBytes / Math.max(128 * MIB, estimated)),
  );
...
export function availableResources(
  ...
  let available = Math.min(
    ...
    memoryAvailable?.[1] ? Number(memoryAvailable[1]) * 1024 : os.freemem(),
  );
