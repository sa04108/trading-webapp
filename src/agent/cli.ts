...
export async function runAgentCli(
  ...
  if (process.platform !== "linux")
    throw new Error("Linux 또는 WSL2에서 실행하세요");
  if (args[0] === "--check") {
    await import("./client.js");
    console.log(
      `quant-agent ${readRuntimeVersions().agentVersion} (commit ${readGitCommitSha()}) linux-${process.arch}`,
    );
    return;
  }
  if (process.getuid?.() === 0)
    throw new Error("에이전트는 root 대신 일반 사용자로 실행하세요");
  const index = args.indexOf("--state");
  const state = path.resolve(
    index >= 0
      ? (args[index + 1] ?? defaultStateDirectory())
      : defaultStateDirectory(),
  );
  const command = args[0] ?? "install";
  if (!["install", "run", "setup", "update"].includes(command))
    throw new Error(
      "사용법: quant-agent [install|run|setup|update] [--state 경로]",
    );
  if (
    command === "update" &&
    !fs.existsSync(path.join(state, "settings.json"))
  ) {
    throw new Error(
      "먼저 quant-agent setup으로 서버 주소와 Agent 토큰을 설정하세요",
    );
  }
  if (
    command === "setup" ||
    ...
    try {
      ...
      const token = (
        await prompt.question("운영 화면에서 발급한 Agent 토큰: ")
      ).trim();
  ...
  if (command === "install") {
    ...
    console.log(
      "백그라운드 에이전트를 시작했습니다. 상태: systemctl --user status quant-agent",
    );
    return;
  }
  ...
  if (command === "update") {
    ...
    if (!result.updated)
      console.log(`이미 게시된 최신 클라이언트입니다: ${result.version}`);
    else {
      console.log(`클라이언트 업데이트 완료: ${result.version}`);
      if (result.restarted)
        console.log("백그라운드 서비스를 새 버전으로 재시작했습니다.");
      else
        console.log(
          `새 클라이언트 실행 파일: ${path.join(installRoot(), "current/quant-agent")}\n직접 실행 중인 agent는 종료한 뒤 이 파일로 run 명령을 다시 실행하세요.`,
        );
    }
    return;
  }
  ...
  const client = new AgentClient(settings, state, async (version) => {
    ...
    await client.stop();
    console.log(
      "클라이언트 업데이트 완료 — 백그라운드 서비스가 새 버전으로 재시작합니다",
    );
    process.exit(75);
  });
  client.start();
