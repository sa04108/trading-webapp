...
export async function withInstallLock<T>(
  ...
  try {
    ...
    if (!stale)
      throw new Error(
        "다른 클라이언트 설치 또는 업데이트가 진행 중입니다. 완료 후 다시 시도하세요.",
        { cause: error },
      );
    fs.unlinkSync(lock);
    descriptor = fs.openSync(lock, "wx", 0o600);
...
function restartRunningService(): boolean {
  ...
  if (
    status.status !== 0 ||
    !/^LoadState=loaded$/m.test(status.stdout) ||
    !/^ActiveState=(active|activating|reloading)$/m.test(status.stdout)
  )
    return false;
  const restart = spawnSync(
    "systemctl",
    ["--user", "restart", "quant-agent.service"],
    { stdio: "inherit" },
  );
  if (restart.status !== 0)
    throw new Error(
      "클라이언트 설치는 완료했지만 서비스 재시작에 실패했습니다. systemctl --user restart quant-agent를 실행하세요.",
    );
  return true;
}
...
export async function updateAgent(
  ...
  return withInstallLock(async () => {
    ...
    if (version === installedVersion())
      return { version, updated: false, restarted: false };
    ...
    try {
      const unpacked = await downloadUpdate(
        settings,
        version,
        directory,
        manifest,
      ...
      const restarted =
        options.restartService === true && restartRunningService();
