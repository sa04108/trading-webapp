...
export class AgentDatasetCache {
  ...
  file(dataset: DatasetManifest): string {
    return path.join(
      this.directory,
      `${dataset.version}-${dataset.sha256}.sqlite`,
    );
  }
  get syncing(): boolean {
  ...
  async synchronize(input: DatasetManifest): Promise<void> {
    ...
    if (
      this.current?.version === manifest.version &&
      this.current.sha256 === manifest.sha256 &&
      this.current.collectionVersion === manifest.collectionVersion
    )
      return;
    if (this.pending) {
      await this.pending;
    ...
    this.pending = this.download(manifest).finally(() => {
      this.pending = null;
      this.abort = null;
    });
    return this.pending;
  ...
  private async download(manifest: DatasetManifest): Promise<void> {
    ...
    if (fs.existsSync(file)) {
      ...
      if (
        fs.statSync(file).size === manifest.bytes &&
    ...
    if (free.bavail * free.bsize < manifest.bytes * 1.1)
      throw new Error("계산 데이터 다운로드 공간이 부족합니다");
    const temporary = `${file}.part`;
    this.abort = new AbortController();
    try {
      const response = await fetch(
        `${this.settings.serverUrl}/api/agents/datasets/${manifest.version}`,
        {
      ...
      const verify = new Transform({
        transform(chunk: Buffer, _encoding, callback) {
          ...
          if (bytes > manifest.bytes) {
            callback(new Error("데이터 파일이 명세보다 큽니다"));
            return;
          }
          hash.update(chunk);
          callback(null, chunk);
  ...
  private validate(file: string, manifest: DatasetManifest): void {
    ...
    try {
      ...
      if (
        identity.datasetId !== manifest.datasetId ||
        identity.revision !== manifest.sourceRevision
      )
        throw new Error("계산 데이터 식별자 또는 버전 불일치");
      if (sqlite.pragma("quick_check", { simple: true }) !== "ok")
        throw new Error("계산 데이터 무결성 검사 실패");
  ...
  prune(pinnedFiles: ReadonlySet<string>): void {
    for (const name of fs.readdirSync(this.directory)) {
      ...
      if (
        (this.current && file === this.file(this.current)) ||
        pinnedFiles.has(file)
      )
        continue;
