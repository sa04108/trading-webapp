...
export function activate(
  ...
  fs.mkdirSync(path.join(destination, "releases"), {
    recursive: true,
    mode: 0o700,
  });
  ...
  if (path.resolve(source) !== release && !fs.existsSync(release)) {
    ...
    command(path.join(staging, "bin/node"), [
      path.join(staging, "dist/agent/main.js"),
      "--check",
    ]);
    fs.renameSync(staging, release);
  }
  verifyPackage(release, version);
  command(path.join(release, "bin/node"), [
    path.join(release, "dist/agent/main.js"),
    "--check",
  ]);
...
function verifyPackage(source: string, version: string): void {
  if (!/^[a-f0-9]{64}$/.test(version))
    throw new Error("클라이언트 버전 형식이 올바르지 않습니다");
  if (
    !fs.existsSync(path.join(source, "bin/node")) ||
    !fs.existsSync(path.join(source, "quant-agent"))
  )
    throw new Error("빌드된 Linux 클라이언트 패키지에서 설치하세요");
  const metadata = parseRuntimeVersions(
    JSON.parse(
      fs.readFileSync(path.join(source, "dist/runtime-versions.json"), "utf8"),
    ),
  );
  if (metadata.agentVersion !== version)
    throw new Error("클라이언트 패키지의 버전이 다릅니다");
}
...
export function installService(
  ...
  fs.writeFileSync(path.join(unitDirectory, "quant-agent.service"), unit, {
    mode: 0o600,
  });
  ...
  const linger = spawnSync(
    "loginctl",
    ["enable-linger", os.userInfo().username],
    { stdio: "ignore" },
  );
  if (linger.status !== 0)
    console.log(
      "로그아웃 후 상시 실행 설정을 완료하지 못했습니다. 관리자가 loginctl enable-linger 사용자명으로 활성화하세요.",
    );
}
...
export async function fetchClientManifest(
  ...
  if (!response.ok)
    throw new Error(`클라이언트 명세 조회 실패: HTTP ${response.status}`);
...
export async function downloadUpdate(
  ...
  if (manifest.runnerVersion !== version)
    throw new Error("운영 서버와 게시된 클라이언트 버전이 다릅니다");
  const client = manifest.clients.find(
    (entry) =>
      entry.arch === process.arch &&
      entry.file === `quant-agent-linux-${process.arch}.tar.gz`,
  );
  if (!client)
    throw new Error("현재 CPU 아키텍처의 클라이언트가 게시되지 않았습니다");
  fs.mkdirSync(directory, { recursive: true, mode: 0o700 });
  const archive = path.join(directory, "client.tar.gz.part");
  const result = await fetch(
    `${settings.serverUrl}/api/agents/client/${client.file}`,
    {
  ...
  if (!result.ok || !result.body)
    throw new Error(`클라이언트 다운로드 실패: HTTP ${result.status}`);
  const hash = createHash("sha256");
  let bytes = 0;
  const verify = new Transform({
    transform(chunk: Buffer, _encoding, callback) {
      bytes += chunk.length;
      if (bytes > client.bytes) {
        callback(new Error("클라이언트 파일 크기 초과"));
        return;
      }
      hash.update(chunk);
      callback(null, chunk);
  ...
  if (bytes !== client.bytes || hash.digest("hex") !== client.sha256)
    throw new Error("클라이언트 다운로드 무결성 검사 실패");
  ...
  if (
    ...
    throw new Error("클라이언트 압축 파일 경로가 올바르지 않습니다");
  ...
  command(path.join(unpacked, "bin/node"), [
    path.join(unpacked, "dist/agent/main.js"),
    "--check",
  ]);
  return unpacked;
}
