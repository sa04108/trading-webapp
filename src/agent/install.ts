...
export function activate(source: string, version: string, destination = installRoot()): void {
  ...
  for (const name of fs.readdirSync(path.join(destination, 'releases'))) {
    ...
    if (/^[a-f0-9]{64}$/.test(name) && candidate !== release && candidate !== previous) fs.rmSync(candidate, { recursive: true, force: true });
  }
}

function verifyPackage(source: string, version: string): void {
  if (!/^[a-f0-9]{64}$/.test(version)) throw new Error('클라이언트 버전 형식이 올바르지 않습니다');
...
const packageManifestSchema = z.object({ runnerVersion: z.string().regex(/^[a-f0-9]{64}$/), clients: z.array(z.object({ arch: z.enum(['x64', 'arm64']), file: z.string().regex(/^quant-agent-linux-(x64|arm64)\.tar\.gz$/), sha256: z.string().regex(/^[a-f0-9]{64}$/), bytes: z.number().int().positive() })) });
