...
import {
  dataDatabasePath, datasetIdentity, initializeDatabaseIdentity, migrateDatabaseRole, tableExists,
...
export function openDatabase(databasePath: string, options: DatabaseOpenOptions = {}): DatabaseHandle {
  const dataPath = options.dataPath ?? dataDatabasePath(databasePath);
  if (options.dataReadonly && !fs.statSync(dataPath).isFile()) throw new Error('계산 스냅샷 파일이 없습니다');
  ...
  try {
    ...
    const expected = existing
      ? (sqlite.prepare('SELECT dataset_id AS id FROM operational_database_state WHERE singleton = 1').get() as { id: string } | undefined)?.id
      : undefined;
    ...
    migrateDatabaseRole(sqlite, options.dataReadonly ? 'agent' : 'operations', 'main');
    if (dataPath !== ':memory:' && !options.dataReadonly) fs.mkdirSync(path.dirname(path.resolve(dataPath)), { recursive: true });
    if (options.dataReadonly) {
      // 바인딩이 SQLite URI를 지원하지 않으므로 파일 권한으로 읽기 전용을 보장한다.
      // root처럼 권한을 우회할 수 있는 실행 환경도 스냅샷 쓰기 가능 여부로 거부한다.
      let writable = true;
      try { fs.accessSync(dataPath, fs.constants.W_OK); } catch { writable = false; }
      if (writable) throw new Error('계산 스냅샷은 쓰기 권한이 없는 파일이어야 합니다');
    }
    sqlite.prepare('ATTACH DATABASE ? AS data').run(dataPath);
    let datasetId: string;
    if (options.dataReadonly) {
      datasetId = datasetIdentity(sqlite).datasetId;
      if (expected && expected !== datasetId) throw new Error('작업 DB와 계산 DB의 데이터셋 식별자가 다릅니다');
    } else {
    ...
    sqlite.prepare('INSERT OR IGNORE INTO operational_database_state (singleton, dataset_id) VALUES (1, ?)').run(datasetId);
    return { db: drizzle(sqlite, { schema }), sqlite, dataPath, close: () => sqlite.close() };
  } catch (error) {
    sqlite.close();
    throw error;
  }
