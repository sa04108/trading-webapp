...
export const backtestPreparationJobs = sqliteTable(
  "backtest_preparation_jobs",
  {
    ...
    lifecycleManaged: integer("lifecycle_managed", { mode: "boolean" })
      .notNull()
      .default(false),
  ...
  (table) => [
    index("preparation_jobs_hash_idx").on(table.requestHash, table.status),
  ],
);

/** Only a fully validated, unchanged data revision may reuse a completed preview. */
export const preparationPreviewCache = sqliteTable(
...
export const operationalDatabaseState = sqliteTable(
