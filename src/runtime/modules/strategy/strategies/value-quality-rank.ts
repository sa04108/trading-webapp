...
export type ValueQualityRankParameters = z.infer<
  typeof valueQualityRankParameters
>;
...
const CASH_FIELDS: readonly FundamentalField[] = [
...
export function computeValueQualityMetrics(
  ...
  if (
    snapshot.latestPeriodKey === null ||
    quarterOrdinal(snapshot.latestPeriodKey) === null
  ) {
    return null;
  }
  ...
  const accountFields =
    marketCapKrw === undefined
      ...
      : CONSULTED_FIELDS;
  const accountQuarters = accountFields
    .map((field) => snapshot.periodKeyOf(field))
  ...
  const enterpriseValue =
    marketCap +
    sumFields(snapshot, DEBT_FIELDS) -
    sumFields(snapshot, CASH_FIELDS);
  ...
  onBars(
    ...
    for (const symbol of state.symbols) {
      ...
      if (
        ...
        continue;
      const marketCapKrw =
        context.selectionMetric(symbol)?.marketCapKrw ?? undefined;
