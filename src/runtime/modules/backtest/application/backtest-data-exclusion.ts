...
export function backtestDataExclusionKey(
  exclusion: BacktestDataExclusion,
): string {
  return [
    exclusion.symbol,
    exclusion.category,
...
function categoryLabel(category: BacktestDataExclusionCategory): string {
  switch (category) {
    case "KRX_CLASSIFICATION":
      return "KRX 종목 분류";
    case "KRX_SELECTION_METRIC":
      return "KRX 선정 지표";
    case "KRX_PRICE":
      return "KRX 가격";
    case "DART_FINANCIAL":
      return "DART 재무";
    case "DART_CORPORATE_ACTION":
      return "자본변동";
  }
}
...
export function backtestDataExclusionWarnings(
  ...
  for (const exclusion of exclusions) {
    ...
    if (
      !values.some(
        (value) =>
          backtestDataExclusionKey(value) ===
          backtestDataExclusionKey(exclusion),
      )
    ) {
  ...
  return [...grouped.values()]
    ...
    .map((values) => {
      ...
      const causes = values
        ...
        .join("; ");
      return (
        `${categoryLabel(first.category)} 정보를 온전히 확보할 수 없어 종목 ` +
