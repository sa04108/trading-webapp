...
export type FinancialExecutionWindows = ReadonlyMap<
  string,
  readonly CandleTimeWindow[]
>;
...
export function buildFinancialExecutionWindows(input: {
  ...
  readonly schedule: readonly Pick<
    LegacyUniverseScheduleEntry,
    "rebalanceDate" | "symbols"
  >[];
  ...
  for (let index = 0; index < schedule.length; index += 1) {
    ...
    for (const symbol of entry.symbols) {
      ...
      const executableToTsMs =
        delistedTsMs === undefined
          ? segmentToTsMs
...
export function financialFactCutoffsFromCoverage(input: {
  ...
  return input.candles.getLastTsInWindows(
    buildFinancialExecutionWindows(input),
  );
}
...
export function financialFactCutoffsFromCandles(input: {
  ...
  for (const candle of input.candles) {
    ...
    if (symbolWindows === undefined || !containsTs(symbolWindows, candle.tsMs))
      continue;
    const existing = result.get(candle.symbol);
    if (existing === undefined || candle.tsMs > existing)
      result.set(candle.symbol, candle.tsMs);
  }
  return result;
}

function containsTs(
  windows: readonly CandleTimeWindow[],
  tsMs: number,
): boolean {
  let low = 0;
