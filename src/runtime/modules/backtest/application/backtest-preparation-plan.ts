...
export function buildBacktestPreparationPlan(input: {
  ...
  const universeLookback = request.universeRule.stages.some(
    ...
    ? 4
    : 0;
  const strategyLookback =
    strategy.dataRequirements?.fundamentalLookbackQuarters ?? 0;
  const fundamentalLookbackQuarters = Math.max(
    universeLookback,
    strategyLookback,
  );
  ...
  const parsedParameters = strategy.parameterSchema.safeParse(
    request.parameters,
  );
  ...
  const priceWarmupBars =
    strategy.dataRequirements?.priceWarmupBars?.(parsedParameters.data) ?? 0;
  ...
  if (priceWarmupBars > 0) {
    ...
    const strategyRange = {
      from: addCalendarDays(
        request.period.from,
        -(Math.ceil(priceWarmupBars) * 2 + 14),
      ),
  ...
  const executionWarmupBars = Math.ceil(
    Math.max(0, priceWarmupBars, declineWarmupBars),
  );
  const conservativeExecutionFrom =
    executionWarmupBars === 0
      ? request.period.from
  ...
  const actionExecutionFrom =
    plannedExecutionFrom < conservativeExecutionFrom
  ...
  return {
    ...
    rebalanceDates: computeRebalanceDates(
      request.period,
...
export function backtestPreparationRequestHash(
  request: Pick<
    BacktestRequest,
    "period" | "universeRule" | "strategyId" | "parameters"
  >,
  ...
  return createHash("sha256")
    .update(JSON.stringify(canonicalize(canonicalInput)))
    .digest("hex");
}
