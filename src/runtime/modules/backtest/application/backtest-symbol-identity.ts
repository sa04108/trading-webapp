...
type IdentityMaster = Pick<SymbolMasterService, "readIdentitySnapshot">;
...
export function assertPinnedScheduleHash(
  schedule: unknown,
  expectedScheduleHash: unknown,
): void {
  if (
...
export function assertSafeIdentitySelections(
  ...
  const validation = validateSymbolIdentityLifetime(
    selections,
    snapshot.versions,
  );
...
export function assertSafePinnedScheduleIdentities(
  ...
  for (const entry of schedule) {
    ...
    if (
      ...
      entry.symbols.some((symbol) => typeof symbol !== "string")
    ) {
      throw new UnsafeBacktestSymbolIdentityError(
  ...
  const legacyShortCodes = [
    ...new Set(legacyReferences.map((reference) => reference.shortCode)),
  ];
  const snapshot = deps.symbolMaster.readIdentitySnapshot(
    [
      ...modernSelections.map((selection) => selection.shortCode),
      ...legacyShortCodes,
    ],
  ...
  if (legacyShortCodes.length > 0) {
    const inferred = inferUniqueSymbolIdentities(
      legacyShortCodes,
      snapshot.versions,
    );
    ...
    if (conflict !== undefined) {
      throw new UnsafeBacktestSymbolIdentityError(
        identityInferenceConflictMessage(conflict),
      );
    }
...
export function assertPinnedScheduleExecutionDates(
  ...
  for (const entry of schedule) {
    ...
    const rebalanceDate = (entry as { readonly rebalanceDate?: unknown })
      .rebalanceDate;
    if (
...
function assertRegisteredIdentities(
  ...
  const registrationsByShort = new Map(
    registrations.map((row) => [row.code, row]),
  );
  const registrationsByStandard = new Map(
    registrations.flatMap((row) =>
      row.standardCode === null ? [] : [[row.standardCode, row] as const],
    ),
  );
...
export function identityConflictMessage(
  conflict: SymbolIdentityConflict,
): string {
  if (conflict.kind === "SHORT_CODE_REUSED") {
    return (
      `단축코드 ${conflict.shortCode}이 여러 표준코드(` +
  ...
  if (conflict.kind === "STANDARD_CODE_REASSIGNED") {
    return (
      `KRX 표준코드 ${conflict.standardCode}가 여러 단축코드(` +
      `${conflict.shortCodes.join(", ")})를 사용한 이력이 있습니다. ` +
      "현재 실행 엔진은 코드 변경 전후 상태를 연결할 수 없어 실행을 차단했습니다."
    );
  }
  const active =
    conflict.activePairs.length === 0
      ? "해당 날짜에 연결된 pair 없음"
      : conflict.activePairs
          .map((pair) => `${pair.shortCode}/${pair.standardCode}`)
          .join(", ");
  return (
    `${conflict.effectiveDate}에 ${conflict.shortCode}/${conflict.standardCode} identity가 ` +
...
export function identityInferenceConflictMessage(
  ...
  if (conflict.kind === "SHORT_CODE_UNKNOWN") {
    return (
      `옛 작업의 단축코드 ${conflict.shortCode}에 대한 KRX 표준 identity 이력이 없습니다. ` +
      "미리보기를 다시 실행해 새 백테스트를 생성하세요."
    );
  }
