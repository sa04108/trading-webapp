...
import { AgentDataRequired, agentLeaseSchema, type AgentLease } from '../../shared/agent-protocol.js';
...
async function run(input: { lease: AgentLease; jobPath: string; dataPath: string }): Promise<void> {
  ...
  try {
    ...
    stop = () => { orchestrator?.cancel(input.lease.jobId); };
    onMessage = (message: { type: string }) => { if (message.type === 'cancel') stop?.(); };
