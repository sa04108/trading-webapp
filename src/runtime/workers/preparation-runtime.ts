...
import type { FactSyncRequest, FactSyncReport } from '../modules/facts/application/fact-sync-port.js';
...
export function createPreparationWorkerRuntime(database: DatabaseHandle, options: { readonly collectionVersion: string; readonly onJobUpdated?: (jobId: string) => void }): BacktestPreparationOrchestrator {
  ...
  const factCoverage = new SqliteFactCoverageStore(database.db, { collectionVersion });
  const actionCoverage = new SqliteCorporateActionCoverageStore(database.db, { collectionVersion });
  const selection = new SelectionMetricRepository(database.db, { collectionVersion });
  const symbolMaster = new SymbolMasterService({ db: database.db, clock, logger, collectionVersion, source: {
