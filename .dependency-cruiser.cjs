  ...
  forbidden: [
    {
      ...
      severity: 'error',
      comment: 'agent와 공유 계산 코드는 서버 전용 구현에 의존하지 않는다',
      from: { path: '^src/(agent|runtime)' },
      to: { path: '^src/(server|workers|web)' },
    },
    {
    ...
    },
    {
      ...
      from: { path: 'src/(server|runtime)/modules/market-data' },
      to: { path: 'src/(server|runtime)/modules/broker' },
    },
    {
      ...
      from: { path: 'src/(server|runtime)/modules/market-data' },
      to: { path: 'src/(server|runtime)/modules/facts' },
    },
    {
      ...
      from: { path: 'src/(server|runtime)/modules/market-data' },
      to: { path: 'src/(server|runtime)/modules/notification' },
    },
    {
      name: 'backtest-no-notification',
      severity: 'error',
      comment: 'backtest → notification 금지 — container 가 listener 로 잇는다',
      from: { path: 'src/(server|runtime)/modules/backtest' },
      to: { path: 'src/(server|runtime)/modules/notification' },
    },
    {
      ...
      from: { path: 'src/(server|runtime)/modules/strategy' },
      to: { path: 'src/(server|runtime)/modules/broker' },
    },
    {
    ...
    },
    {
      ...
      from: { path: 'src/(server|runtime)/modules/facts' },
      to: { path: 'src/(server|runtime)/modules/broker' },
    },
    {
      ...
      from: { path: '^src/web' },
      to: { path: '^src/(server|runtime)' },
    },
    {
      name: 'server-no-web',
      severity: 'error',
      comment: '서버가 웹 코드를 import 금지',
      from: { path: '^src/(server|runtime|workers)' },
      to: { path: '^src/web' },
    },
  ],
  options: {
