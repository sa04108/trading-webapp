...
describe('createDartFactSource — 영속 원문 snapshot 재처리', () => {
  it('전체 응답을 보존하고 다음 실행은 API key·corp_code 요청 없이 같은 팩트를 만든다', async () => {
    ...
    expect(liveSource.countRawSnapshotMisses?.(liveRequest, true)).toBe(0);
  ...
  it('REFRESH만으로 정상 원문을 우회하거나 외부 호출을 허용하지 않는다', async () => {
    const rawSnapshots = new MemoryRawSnapshotStore();
    rawSnapshots.put({
      symbol: '005930',
    ...
    expect(liveCalls - callsBeforeRefresh).toBe(0);
    expect(result.facts.find((fact) => fact.field === 'CURRENT_ASSETS')?.value).toBe(1);
    expect(rawSnapshots.get({
      ...
      reportCode: '11013',
      fsDiv: 'CFS',
    })?.payload).toMatchObject({ message: '과거' });
  });

  it('list가 없는 깨진 000 snapshot은 외부 호출 없이 차단한다', async () => {
    const rawSnapshots = new MemoryRawSnapshotStore();
    ...
    await expect(source.fetchFinancials({
      ...
      consolidated: true,
      rawSnapshotPolicy: 'PREFER_CACHE',
    })).rejects.toThrow(/PARSER_INCOMPATIBLE/);
...
describe('createDartFactSource — 종목별 호출에서도 캐시가 공유된다', () => {
  ...
  it.each(['AUTHORIZE', 'ATTEMPT'] as const)('고유번호 %s 승인 차단은 HTTP와 quota를 기록하지 않는다', async (stage) => {
    let physical = 0;
    let recorded = 0;
    const source = createDartFactSource({baseUrl:'https://dart.test',apiKey:'k'},LOGGER,{
      usage:{recordCall:()=>++recorded,callsUsed:()=>recorded,maxCallsUsed:()=>recorded,quotaExceeded:()=>false,reportQuotaExceeded:()=>true},
      requestPolicy:{authorize:(key)=>{
        const corp = key.endpoint === '/api/corpCode.xml';
        if (corp && stage === 'AUTHORIZE') throw new Error('고유번호 승인 차단');
        return {fingerprint:key.endpoint,reason:'FIRST_ACQUISITION',beforeAttempt(){if(corp && stage === 'ATTEMPT') throw new Error('고유번호 승인 차단');},complete(){}};
      }},
      fetchImpl:(async()=>{physical+=1;throw new Error('HTTP 호출 금지');}) as typeof fetch,
    });
    await expect(source.fetchFinancials({symbols:['005930'],years:[2025],shareYears:[],consolidated:true})).rejects.toThrow('고유번호 승인 차단');
    expect({physical,recorded}).toEqual({physical:0,recorded:0});
  });

  it.each(['PARSE', 'PERSIST', 'SUCCESS'] as const)('고유번호 %s 단계에서는 파싱·보존 완료 뒤에만 승인을 완료한다', async (stage) => {
    let saved: {xml:string;fetchedAtMs:number} | null = null;
    const events: string[] = [];
    const source = createDartFactSource({baseUrl:'https://dart.test',apiKey:'k'},LOGGER,{
      sleep:async()=>{},
      corpCodeSnapshots:{get:()=>saved,put:(xml,fetchedAtMs)=>{
        if(stage === 'PERSIST') throw new Error('보존 실패');
        events.push('PERSIST');saved={xml,fetchedAtMs};
      }},
      requestPolicy:{authorize:(key)=>({fingerprint:key.endpoint,reason:'FIRST_ACQUISITION',beforeAttempt(){},complete(){if(key.endpoint === '/api/corpCode.xml') events.push('COMPLETE');}})},
      fetchImpl:(async(url)=>String(url).includes('corpCode.xml')
        ? new Response(new Uint8Array(storedZip('CORPCODE.xml',stage === 'PARSE' ? '<result/>' : CORP_XML)),{status:200})
        : jsonResponse({status:'013'})) as typeof fetch,
    });
    const result=source.fetchFinancials({symbols:['005930'],years:[2025],shareYears:[],consolidated:true});
    if(stage === 'SUCCESS') {await result;expect(events).toEqual(['PERSIST','COMPLETE']);}
    else {await expect(result).rejects.toThrow();expect(events).toEqual([]);}
  });
...
describe('발견 공시의 보고서 단위 원문 갱신', () => {
  it('승인한 한 보고서만 호출하고 013이면 원문·checkpoint를 보존하여 반복 호출을 막는다', async () => {
    const rawSnapshots = new MemoryRawSnapshotStore();
    const key = {symbol:'005930',endpoint:'FINANCIAL_STATEMENT',businessYear:2025,reportCode:'11013',fsDiv:'CFS'} as const;
    const old = {status:'000',list:[{rcept_no:'20250515000001'}]};
    rawSnapshots.put(key,old,1);
    let retryAfterMs: number | null = null;
    let calls = 0;
    let applied = 0;
    const reasons: string[] = [];
    const source = createDartFactSource({baseUrl:'https://dart.test',apiKey:'k'},LOGGER,{
      rawSnapshots, corpCodeResolver:STUB_RESOLVER, clock:{now:()=>Date.UTC(2026,0,1)},
      pendingFilings:{
        get: (candidate) => candidate.reportCode === '11013' ? {receiptNo:'20250516000001',discoveredAtMs:2,status:'PENDING',retryAfterMs} : null,
        markApplied:()=>{applied+=1;},
        markPendingPublication:(_key,_receipt,at)=>{retryAfterMs=at;},
      },
      requestPolicy:{authorize:(request,state)=>{
        expect(request.parameters.reportCode).toBe('11013');
        reasons.push(state.kind);
        return {fingerprint:'test',reason:'SOURCE_CHANGE_CONFIRMED',beforeAttempt(){},complete(){}};
      }},
      fetchImpl:(async()=>{calls+=1; return jsonResponse({status:'013'});}) as typeof fetch,
    });
    const request = {symbols:['005930'],years:[2025],shareYears:[],consolidated:true};
    await expect(source.fetchFinancials(request)).rejects.toThrow(/PENDING_PUBLICATION/);
    await expect(source.fetchFinancials({...request})).rejects.toThrow(/PENDING_PUBLICATION/);
    expect(calls).toBe(1);
    expect(reasons).toEqual(['REPLACEMENT']);
    expect(rawSnapshots.get(key)?.payload).toEqual(old);
    expect(applied).toBe(0);
  });
});


it('서로 다른 실행 scope도 같은 영속 저장소의 미수집 원문 HTTP를 공유한다', async () => {
  const rawSnapshots = new MemoryRawSnapshotStore();
  let calls = 0;
  const source = createDartFactSource({baseUrl:'https://dart.test',apiKey:'k'},LOGGER,{
    rawSnapshots,corpCodeResolver:STUB_RESOLVER,sleep:async()=>{},clock:{now:()=>Date.UTC(2026,0,1)},
    fetchImpl:(async()=>{calls+=1; await Promise.resolve(); return jsonResponse({status:'013'});}) as typeof fetch,
  });
  const request = {symbols:['005930'],years:[2025],shareYears:[],consolidated:true};
  const [first, second] = await Promise.all([
    source.fetchFinancials({...request,rawSnapshotScope:{}}),
    source.fetchFinancials({...request,rawSnapshotScope:{}}),
  ]);
...
it('저장 원문의 법인과 로컬 고유번호가 다르면 HTTP 없이 차단한다', async () => {
  const rawSnapshots = new MemoryRawSnapshotStore();
  rawSnapshots.put({symbol:'005930',endpoint:'FINANCIAL_STATEMENT',businessYear:2025,reportCode:'11013',fsDiv:'CFS'},
    {status:'000',list:[{corp_code:'00126380'}]},1);
  let requests = 0;
  const source = createDartFactSource({baseUrl:'https://dart.test',apiKey:'k'},LOGGER,{
    rawSnapshots,corpCodeResolver:{lookup:()=> '99999999',resolve:async()=>{requests+=1;return '99999999';}},
    fetchImpl:(async()=>{requests+=1;throw new Error('외부 호출 금지');}) as typeof fetch,
  });
  await expect(source.fetchFinancials({symbols:['005930'],years:[2025],shareYears:[],consolidated:true})).rejects.toThrow(/IDENTITY_MISMATCH/);
  expect(requests).toBe(0);
});

it('예전 원문에 법인 식별자가 없어도 발견 공시와 저장 매핑이 다르면 추측하여 연결하지 않는다', async () => {
  const rawSnapshots = new MemoryRawSnapshotStore();
  rawSnapshots.put({symbol:'005930',endpoint:'FINANCIAL_STATEMENT',businessYear:2025,reportCode:'11013',fsDiv:'CFS'},
    {status:'013'},1);
  let requests = 0;
  const source = createDartFactSource(null,LOGGER,{
    rawSnapshots,corpCodeResolver:{lookup:()=> '00126380',resolve:async()=>{requests+=1;return '00126380';}},
    pendingFilings:{get:()=>null,getExpectedCorpCode:()=> '99999999',markApplied(){},markPendingPublication(){}},
  });
  await expect(source.fetchFinancials({symbols:['005930'],years:[2025],shareYears:[],consolidated:true})).rejects.toThrow(/IDENTITY_CHANGED/);
  expect(requests).toBe(0);
});
