...
describe('SymbolMasterService.ingestDate — 거래불가일', () => {
  it('정지 행은 krx_non_trading_days 로, 정상 행은 krx_daily_bars 로 간다', async ({ krxApps }) => {
...
describe('SymbolMasterService.ingestDate — 거래불가 커버리지', () => {
  it('정상 수집만으로도 커버로 남고, 하루씩 들어와도 한 구간으로 합쳐진다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
  ...
  it('휴장일도 커버로 남아 앞뒤 거래일 구간이 이어진다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
...
describe('거래불가일 조회', () => {
  it('구간 안의 행만 날짜·코드 오름차순으로 돌려준다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
  ...
  it('요청한 코드만 500개씩 조회하고 중복 제거 뒤 전체 날짜·코드 순서를 유지한다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
    ctx.t.container.database.db.insert(krxNonTradingDays).values([
  ...
  it('빈 코드 범위는 전체 조회가 아니라 즉시 빈 결과다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
    ctx.t.container.database.db.insert(krxNonTradingDays).values({
      date: '2021-06-15', shortCode: '215600', market: 'KOSDAQ', lastClose: 12_100,
    }).run();
  ...
  it('구간 전체를 덮는 커버 행이 있어야 covered 다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
...
describe('거래불가일 백필', () => {
  it('봉·종목 버전·coverage 를 건드리지 않고 거래불가일만 채운다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
  ...
  it('나눠 부르거나 다시 불러도 커버 구간이 겹쳐 쌓이지 않는다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
  ...
  it('한 날짜도 응답을 받지 못하면 커버로 남기지 않는다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
  ...
  it('응답은 받았고 거래불가 종목만 0건이면 커버로 남는다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
  ...
  it('도중에 실패해도 그때까지 처리한 날짜는 커버로 남는다', async ({ krxApps }) => {
    const ctx = await setup(krxApps);
