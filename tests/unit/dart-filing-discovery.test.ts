...
describe("미리보기 시작 시 DART 공시 조회", () => {
  it("동시 요청은 공유하고 같은 날 다시 시작하면 당일까지 새로 확인한다", async () => {
    ...
    try {
      ...
      await Promise.all([discovery.refresh(), discovery.refresh(), new DartFilingDiscovery(options).refresh()]);
      expect(fetchPage).toHaveBeenCalledTimes(1);
      expect(discovery.freshness()).toEqual({lastCheckedAtMs: now(), checkedThrough:"2026-09-20", warning:null});
      await new DartFilingDiscovery(options).refresh();
      expect(fetchPage).toHaveBeenCalledTimes(2);
      expect((fetchPage.mock.calls as unknown as [string,string,number][])[1]?.slice(0,3)).toEqual(["2026-09-19","2026-09-20",1]);
  ...
  it("기존 watermark부터 공백을 분할 조회하고 다음 시작은 최근 경계만 확인한다", async () => {
    const database = openDatabase(":memory:");
    try {
  ...
  it("자정 후 재개는 원래 종료일을 유지하고 다음 작업의 겹친 경계에서 접수를 중복 저장하지 않는다", async () => {
    ...
    try {
      ...
      expect(fetchPage.mock.calls[2]?.slice(1,3)).toEqual(["2026-09-20",2]);
      expect(fetchPage.mock.calls[3]?.slice(0,3)).toEqual(["2026-09-19","2026-09-21",1]);
  ...
  it("새벽에도 명시적 시작은 조회하고 생성·상태 읽기·시간 경과는 조회하지 않는다", async () => {
    const database = openDatabase(":memory:");
    try {
      const fetchPage = vi.fn(async () => ({status:"013"}));
      const discovery = new DartFilingDiscovery({sqlite:database.sqlite,fetchPage,now:()=>Date.parse("2026-09-19T16:00:00Z"),logger:pino({level:"silent"})});
      discovery.freshness();
  ...
  it("같은 날 재확인 실패는 이전 성공 기록을 보존하면서 경고하고 재개한다", async () => {
    const database = openDatabase(":memory:");
    try {
      const fetchPage = vi.fn().mockResolvedValueOnce({status:"013"}).mockRejectedValueOnce(new Error("offline")).mockResolvedValue({status:"013"});
      const discovery = new DartFilingDiscovery({sqlite:database.sqlite,fetchPage,now,logger:pino({level:"silent"})});
      await discovery.refresh();
      await discovery.refresh();
      expect(discovery.freshness()).toEqual({lastCheckedAtMs:now(),checkedThrough:"2026-09-20",warning:"최신 공시 확인 미완료"});
      await discovery.refresh();
      expect(discovery.freshness().warning).toBeNull();
      expect(fetchPage).toHaveBeenCalledTimes(3);
    } finally { database.close(); }
