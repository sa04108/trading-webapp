...
describe('getUniverseAsOf', () => {
  it('validToDate는 미포함이고 같은 날 시작하는 새 버전은 포함한다', async ({ ctx: t }) => {
  ...
  it('폐지일에는 validToDate가 끝난 종목을 유니버스에서 제외한다', async ({ ctx: t }) => {
    const svc = makeService(t);
    insertCoverage(t, '2023-01-01', '2023-12-31');
    insertVersion(t, entry(), '2023-01-02', '2023-02-01');
  ...
  it('폐지 후 재상장하면 부재 구간 뒤의 새 버전만 다시 노출한다', async ({ ctx: t }) => {
    const svc = makeService(t);
    insertCoverage(t, '2023-01-01', '2023-12-31');
    insertVersion(t, entry(), '2023-01-02', '2023-02-01');
    insertVersion(
...
describe('SymbolMasterService.ingestDate selection metrics', () => {
  it('daily trade 의 cap·volume·거래대금을 표준코드 metric 으로 같은 ingest transaction 에 저장한다', async ({ ctx: t }) => {
...
describe('SymbolMasterService.isRangeCovered', () => {
  it('구간 전체를 덮는 커버 구간 하나면 true 다', async ({ ctx: t }) => {
    const svc = makeService(t);
  ...
  it('경계가 정확히 일치해도 true 다', async ({ ctx: t }) => {
    const svc = makeService(t);
  ...
  it('리밸런스 날짜만 커버되고 그 사이가 비어 있으면 false 다', async ({ ctx: t }) => {
    const svc = makeService(t);
  ...
  it('구간 앞부분이 비어 있으면 false 다', async ({ ctx: t }) => {
    const svc = makeService(t);
  ...
  it('구간 뒷부분이 비어 있으면 false 다', async ({ ctx: t }) => {
    const svc = makeService(t);
  ...
  it('커버 구간이 아예 없으면 false 다', async ({ ctx: t }) => {
    const svc = makeService(t);

    expect(svc.isRangeCovered('2026-01-05', '2026-03-05')).toBe(false);
  });

  it('서로 하루 간격으로 이어 붙는 여러 구간이 합쳐서 덮으면 true 다', async ({ ctx: t }) => {
    const svc = makeService(t);
...
describe('SymbolMasterService.delistedEventsBetween', () => {
  it('닫힌 SCD 행에서 shortCode 를 꺼낸다', async ({ ctx: t }) => {
    const svc = makeService(t);
  ...
  it('DELISTED 가 아닌 이벤트는 제외한다', async ({ ctx: t }) => {
    const svc = makeService(t);
    insertCoverage(t, '2020-01-01', '2026-12-31');
  ...
  it('구간 밖에서 폐지된 종목은 제외한다', async ({ ctx: t }) => {
    const svc = makeService(t);
    insertCoverage(t, '2020-01-01', '2026-12-31');
    insertVersion(
      t,
  ...
  it('하루 빠졌다 돌아온 종목은 폐지로 보고하지 않고, 돌아오지 않은 종목만 보고한다', async ({ ctx: t }) => {
    const svc = makeService(t);
    insertCoverage(t, '2020-01-01', '2026-12-31');
  ...
  it('effectiveDate 오름차순, 같은 날짜는 id(=날짜:표준코드:이벤트타입) 순서로 정렬한다', async ({ ctx: t }) => {
    const svc = makeService(t);
    insertCoverage(t, '2020-01-01', '2026-12-31');
    insertVersion(
      t,
  ...
  it('최초 관측 거래일에 닫힌 version은 baseline 폐지로 만들지 않는다', async ({ ctx: t }) => {
    const svc = makeService(t);
  ...
  it('같은 shortCode가 새 표준코드로 재상장해도 옛 표준코드의 실제 폐지는 보존한다', async ({ ctx: t }) => {
    const svc = makeService(t);
    insertCoverage(t, '2020-01-01', '2026-12-31');
    insertVersion(
      t,
  ...
  it('closing SCD의 shortCode가 비어 있으면 폐지를 건너뛰고 경고한다', async ({ ctx: t }) => {
    const svc = makeService(t);
