...
const EVENTS = [
  { id: 1, eventType: 'LISTED' },
  { id: 2, eventType: 'SHARES_CHANGED' },
  { id: 3, eventType: 'DELISTED' },
  { id: 4, eventType: 'SHARES_CHANGED' },
];
...
describe('filterEventsByType', () => {
  ...
  it('고른 종류만 남긴다', () => {
    expect(filterEventsByType(EVENTS, 'SHARES_CHANGED').map((event) => event.id)).toEqual([2, 4]);
  });
