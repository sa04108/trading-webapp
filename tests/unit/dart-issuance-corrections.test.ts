...
describe('applyDartIssuanceCorrections', () => {
  it('033290의 자기주식 이익소각 3건을 가격 보정 대상 감자와 구분한다', () => {
    const rows = [
      row({}),
      row({ isu_dcrs_de: '2017.05.26', isu_dcrs_qy: '1,064,163' }),
      row({ isu_dcrs_de: '2017년 12월 20일', isu_dcrs_qy: '500000' }),
    ];

    expect(applyDartIssuanceCorrections('033290', rows).map(
      (candidate) => candidate.isu_dcrs_stle,
    )).toEqual(['이익소각', '이익소각', '이익소각']);
  });
