...
// 테스트 러너만 주입한다. Vitest 안에서 별도의 node --test 프로세스를 만들지 않는다.
export function registerDeploymentTests({ test, afterEach }) {
  const fixtures = [];
...
if (command === 'journalctl') process.exit(0);
process.exit(97);
`;
    for (const command of ['ssh', 'scp', 'sudo', 'corepack', 'systemctl', 'systemd-run', 'runtime-node', 'journalctl']) executable(`${bin}/${command}`, mock);
    // readiness 재시도마다 Node를 기동하지 않는다. 실제 배포의 60회 반복은 그대로 검증한다.
    const pollMock = String.raw`#!/bin/sh
set -eu
case "$0" in
  */curl)
    [ "$#" -eq 2 ] && [ "$1" = -fsS ] && [ "$2" = http://127.0.0.1:3000/api/v1/health/ready ] || exit 98
    printf '{"command":"curl","args":["-fsS","http://127.0.0.1:3000/api/v1/health/ready"]}\n' >> "$ROOT/trace"
    if [ "$FAIL" = readiness ] && [ "$(readlink "$ROOT/opt/current")" != "$ROOT/opt/releases/old-release" ]; then
      printf readiness > "$ROOT/failed"
      exit 42
    fi
    if [ -f "$ROOT/failed" ] && [ "$FAIL_RECOVERY" = readiness ]; then exit 43; fi
    ;;
  */sleep)
    [ "$#" -eq 1 ] && [ "$1" = 2 ] || exit 98
    printf '{"command":"sleep","args":["2"]}\n' >> "$ROOT/trace"
    ;;
  *) exit 97 ;;
esac
`;
    for (const command of ['curl', 'sleep']) executable(`${bin}/${command}`, pollMock);
    ...
    const execute = (overrides = {}, args = []) => {
      const result = spawnSync('/bin/bash', [`${repo}/scripts/deploy.sh`, ...args], {
        cwd: repo, env: { ...environment, ...overrides }, encoding: 'utf8', timeout: 15_000,
        // 입력을 읽지 않고 종료하는 오류 경로에 데이터를 쓰면 EPIPE가 날 수 있다.
        input: overrides.READ_STDIN === '1' ? 'input-preserved\n' : undefined,
      });
      if (result.error) {
        const log = fs.existsSync(`${root}/deploy.log`) ? fs.readFileSync(`${root}/deploy.log`, 'utf8') : '';
        const traceTail = fs.existsSync(`${root}/trace`) ? fs.readFileSync(`${root}/trace`, 'utf8').trim().split('\n').slice(-10).join('\n') : '';
        throw new Error([
          `배포 테스트 자식 실행 실패: ${result.error.message}`,
          result.stdout, result.stderr, log, `최근 실행 명령:\n${traceTail}`,
        ].filter(Boolean).join('\n'), { cause: result.error });
      }
  ...
  test('Node 조정 코드 없이 source와 Bash 문법 검사를 지원한다', () => {
    ...
    const vitestEntry = fs.readFileSync(new URL('../unit/deploy-script.test.ts', import.meta.url), 'utf8');
    assert.doesNotMatch(vitestEntry, /spawnSync|node:child_process|110_000|120_000|--test/);
    assert.match(vitestEntry, /registerDeploymentTests\(\{ test: it, afterEach \}\)/);
  ...
  for (const phase of ['preflight', 'build', 'mktemp', 'upload', 'connection', 'checksum', 'install', 'stop', 'backup', 'switch', 'prepare', 'start', 'readiness', 'commit']) {
    test(`${phase} 실패 시 기존 코드·두 DB를 유지하거나 복원하고 오류를 로그에 남긴다`, () => {
      ...
      if (phase === 'readiness') {
        // 신규 릴리스 실패 60회와 복원된 릴리스 성공 1회를 모두 실행한다.
        assert.equal(f.trace().filter(e => e.command === 'curl').length, 61);
        assert.equal(f.trace().filter(e => e.command === 'sleep').length, 59);
      }
  ...
  for (const recovery of ['restore', 'start', 'readiness']) {
    test(`rollback ${recovery} 실패 시 복구 코드와 백업 세트를 보존한다`, () => {
      ...
      if (recovery === 'readiness') {
        assert.equal(f.trace().filter(e => e.command === 'curl').length, 60);
        assert.equal(f.trace().filter(e => e.command === 'sleep').length, 59);
      }
    });
  }
