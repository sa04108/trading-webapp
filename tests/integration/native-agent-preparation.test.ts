...
async function seed(ctx: TestApp): Promise<void> {
...
const it = agentBase.extend<{ scenario: TestApp }>({
...
describe('Linux 에이전트 유니버스 실행', () => {
  it('운영 서버가 유니버스를 계산하지 않고 에이전트의 검증 결과를 저장한다', { timeout: 90_000 }, async ({ scenario: ctx }) => {
    await seed(ctx); await seedCorporateActionCoverage(ctx.container, ['005930'], [2024, 2025, 2026]);
  ...
  it('완료 뒤 IPC를 끊어도 닫힌 작업 DB를 다시 읽지 않고 종료한다', { timeout: 45_000 }, async ({ scenario: ctx }) => {
    await seed(ctx);
  ...
  it('설치된 에이전트의 수집 버전이 오래되어도 lease의 수집 버전으로 준비를 완료한다', { timeout: 45_000 }, async ({ scenario: ctx }) => {
    await seed(ctx);
    await seedCorporateActionCoverage(ctx.container, ['005930'], [2024, 2025, 2026]);
  ...
  it('데이터가 부족하면 서버에 수집을 요청하고 새 버전으로 이어가며 실패 횟수를 소비하지 않는다', { timeout: 120_000 }, async ({ scenario: ctx }) => {
    await seed(ctx);
