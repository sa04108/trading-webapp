...
승인 필요 오류는 `BLOCKED`로 남겨 자동 재시도하지 않는다. 공시 접수 후 API 반영 대기
(`PENDING_PUBLICATION`)는 해당 요청만 예약 재시도하며 실패 횟수를 소모하지 않는다.
...
미확인 페이지는 설정 화면의 명시적인 목록 수집 요청으로 처리하며, 본문 수집은 별도의
데이터 요구·승인 큐가 담당한다. 목록 수집 중 새 미리보기는 그 작업을 기다리지 않는다.
