...
`requiresFundamentals` 전략은 전략의 `fundamentalLookbackQuarters`와 요청 기간으로
계산한 연도 전부가 **최종 유니버스 모든 종목**의 coverage에 기록돼야 한다. 일부 종목만
미수집인 채 실행하면 그 종목이 랭킹에서 빠져 낙관적으로 치우칠 수 있으므로
409 `PREPARATION_REQUIRED`로 막고 준비 단계로 되돌린다.
완료된 preparation의 coverage가 나중에 사라지면 cached preview를 무효로 하고 다시
준비한다. 제출 직후 삭제 race, 즉시·재설정 복제, 난수 시드 지연 승격과 worker도 같은
조건을 다시 검사한다.

coverage가 완전해도 실제 재무 행은 0건일 수 있다. 각 종목의 마지막 실행 봉까지 사용할
수 있는 실제 재무 행이 없거나 blocking gap이 남으면 해당 종목을 전 기간 매매 대상에서
제외하고 같은 규칙으로 차순위를 다시 고른다. 종목과 원인은 preview·결과 경고에 남긴다.
여기서 마지막 실행 봉은 단순한 기간 내 마지막 raw 봉이 아니라, 고정 schedule에서 그
종목이 실제 멤버인 구간이면서 최초 상장폐지 이전인 마지막 유효 봉이다. `SPLIT_RATIO`와
이 경계 뒤 공시는 실제 재무 보유로 세지 않는다. 모든 후보가 같은 이유로 제외돼 일정이
비면 유효한 매매 대상을 만들 수 없으므로 준비가 실패한다(D-092).
...
blocking gap은 coverage 완료 여부와 별도로 종목 제외 사유가 되고, 전략이 소비하지 않는
미매핑 계정만 informational로 남긴다. 재수집은 해당 연도 재무 snapshot을 원자 교체한다.
...
- **커버리지 없음 → 먼저 수집, 그래도 종목별로 없으면 제외.** 준비 작업이 필요한
  연도를 수집한다. 정상 응답 뒤에도 특정 종목의 coverage를 만들 수 없으면 그 종목을
  전 기간 제외하고 차순위를 다시 고른다. 인증·한도·네트워크·전체 응답 훼손처럼
  대상 범위를 신뢰할 수 없는 전역 실패는 종목별 제외로 숨기지 않는다(D-092).
- **자본변동 정보 불완전 → 종목 제외.** 실행 구간의 KRX 상장주식수 변경과 DART
  기준일 정렬 창 안에서 연결되는 blocking gap, 또는 DART 자본변동을 KRX 실제 변경일과
  정렬할 수 없는 경우에는 해당 종목을 전 기간 매매 대상에서 제외하고 차순위를 다시
  계산한다. 종목·연도·원문 날짜·사유는 결과 경고에 남긴다.
  KRX 변경이 없는 사건 없는 연도·상장 전 기준값은 통과한다(D-081·D-090·D-091). 최신 DART
  누적표의 모호한 분할 수량이 전후 분기 주식수와 모순되면, 같은 사건의 과거 보고서 행 중 전후 주식수와
  정확히 일치하는 행만 복구한다. 그런 근거도 없으면 잘못된 비율 대신 blocking gap으로
  남긴다(D-084).
...
기록에만 남긴다. 미리보기·수집·기간 검증의 버전 입력과 설치 기준은
...
- 수집 범위는 요청한 기간과 최소 warm-up 뿐이다 — 연도·종목을 지정해 미리 돌리는
  절차는 없다.
- DART 일일 호출 한도(40,000건, rate limiter 120ms 간격)나 KRX 엔드포인트 한도에 닿아
  대기하는 것은 실패가 아니다. 다음 KST 날짜가 되면 자동으로 이어서 수집한다. KRX는
  이미 coverage로 확인된 날짜를 다시 요청하지 않는다.
- DART·KRX 실제 HTTP 호출 수는 KST 날짜와 공급자 한도 scope별 SQLite 원장에 기록해
  같은 날 서버가 재시작돼도 보존한다. 공급자 한도 초과는 알림 센터 오류 기록으로 남긴다.
...
다음 봉에서 기존 보유분을 포함한 목표 종목의 부족 비중을 채운다. 따라서 두 리밸런스
활성화 사이에는 비리밸런스 실제 거래 봉이 하나 이상 필요하다. 서버는 확정 유니버스의
실제 일봉 타임라인에서 이 조건을 검사하고, 만족하지 않으면 제출 전에 422로 거부한다.
현재 버전은 2.2.1이다.
...
동일가중 재조정과 실제 거래 봉 간격 검증을 공유하며 현재 버전은 2.2.1이다.
