...
- **커버리지 없음 → 400.** 우회 수단을 두지 않는다. 수집하거나 돌리지
  않거나 둘 중 하나다.
- **gap 있음 → KRX 변경과 대조.** 실행 구간의 KRX 상장주식수 변경과 DART 기준일
  정렬 창 안에서 연결되는 blocking gap만 상세 원인과 함께 차단한다. KRX 변경이 없는
  사건 없는 연도·상장 전 기준값은 통과한다(D-081). 최신 DART 누적표의 모호한 분할
  수량이 전후 분기 주식수와 모순되면, 같은 사건의 과거 보고서 행 중 전후 주식수와
