...
coverage가 완전해도 실제 재무 행은 0건일 수 있다. 일부 종목만 실제 행이 0건이면 정상
무공시일 수 있으므로 실행을 허용하되 랭킹에서 제외하고 이름을 경고한다. 유니버스 전체에
각 종목의 마지막 실행 봉까지 사용할 수 있는 실제 재무 행이 하나도 없으면 422로 막고
수집 gap 확인 또는 기간·유니버스·전략 조정을 안내한다. 여기서 마지막 실행 봉은 단순한
기간 내 마지막 raw 봉이 아니라, 고정 schedule에서 그 종목이 실제 멤버인 구간이면서 최초
상장폐지 이전인 마지막 유효 봉이다. `SPLIT_RATIO`와 이 경계 뒤 공시는 실제 재무 보유로
...
- **커버리지 없음 → 400.** 우회 수단을 두지 않는다. 수집하거나 돌리지
  않거나 둘 중 하나다.
- **gap 있음 → KRX 변경과 대조 후 종목 제외.** 실행 구간의 KRX 상장주식수 변경과
  DART 기준일 정렬 창 안에서 연결되는 blocking gap은 해당 종목을 전 기간 매매 대상에서
  제외하고 차순위를 다시 계산한다. 종목·연도·원문 날짜·사유는 결과 경고에 남긴다.
  KRX 변경이 없는 사건 없는 연도·상장 전 기준값은 통과한다(D-081·D-090). 최신 DART
