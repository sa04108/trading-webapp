...
부모의 내부 제어 오류(`cancellationReason: PROCESS_CONTROL_ERROR`)로 중단한 작업은 워커가 DB나 IPC에 `CANCELLED`를 남겨도 실패로 보고한다. 실제 취소 요청은 기존대로 취소가 우선하며, 제어 오류와 워커의 취소 기록은 진단에 함께 보존한다.
