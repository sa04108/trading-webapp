...
서버 시작이나 시간 경과로 자동 조회하지 않으며 `DART_DISCOVERY_HOUR_KST` 설정은 없다.
...
설정 화면의 「미확인 공시 목록 수집」은 남은 목록만 별도로 처리한다. 1회 작업은 최대
100회·30초로 제한하며 잔여 checkpoint가 있으면 다시 요청할 수 있다. 당시 열린 날짜의
페이지는 다음 날에도 첫 페이지부터 다시 읽어 추가 접수를 놓치지 않는다. 이미 닫힌 기간은
저장된 커서부터 이어 읽는다. 이 수집이 실행 중이어도 새 미리보기는 대기하지 않는다.
...
승인이 필요한 본문 교체는 기존 승인 정책으로 처리한다.
