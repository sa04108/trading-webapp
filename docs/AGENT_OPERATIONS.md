...
시작 기준은 128 MiB와 요청 봉 수×1280 중 큰 값이다. 로컬은 단일 워커의 실제 RSS를
감시하므로 과거 다른 작업의 최대 RSS로 다음 작업을 영구 차단하지 않는다. 원격 agent는
기존 관측 RSS×1.35, 최소 256 MiB 또는 RAM 10% 여유분, 병렬 배정과 일반 취소 경로를 유지한다.
128 MiB는 모든 작업의 완료를 보장하는 예산이 아니다. 정상 종료는 서버 자식 종료를 확인한
