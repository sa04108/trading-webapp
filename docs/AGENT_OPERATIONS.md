...
DART 목록 작업은 `DART_DISCOVERY_HOUR_KST`(0~23, 기본 6) 이후 KST 하루 1회 수행한다.
완료 작업은 재시작 후 반복하지 않고 미완료 페이지부터 이어간다. 전일까지의 목록 경계를
확인하며 당일 접수는 다음 일일 작업 대상이다. 초기 조회 범위와 완료 checkpoint를 혼동해
