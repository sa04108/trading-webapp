...
수집 파서만 바뀌면 저장 원문을 우선 재해석한다. 실행 버전 변경은 외부 재수집 권한이 아니다.
...
## 공급자 원문·최신성·승인 운영 (D-096)

원문과 공시 조회 checkpoint·승인 계획·HTTP 시도 원장은 서버 운영 DB에 보관한다.
계산 DB와 함께 백업·복원하며, 원문·coverage를 지워 재수집을 유도하지 않는다. 정상
legacy 자료는 원문이 없어도 현재 계산에 충분하면 재사용한다. 필요한 원문·필드가 없거나
손상·정정이 확인되면 설정 화면의 「외부 데이터 확인·수집 승인」에서 대상 API·날짜·보고서,
사유·근거·시도 한도를 확인해 승인하거나 취소한다. 승인은 해당 요청 키와 근거에만 적용된다.
시도 한도를 모두 사용했으면 사용자가 추가 최대 5회 시도를 승인할 수 있고 기존 시도 기록은 남는다.

DART 목록은 사용자가 미리보기를 시작할 때마다 KST 당일까지 확인한다. 서버 시작이나
시간 경과로 자동 조회하지 않으며 `DART_DISCOVERY_HOUR_KST` 설정은 제거했다.
동시 미리보기는 진행 중인 목록 확인·대조를 공유한다. 다음 시작은 이전 완료 경계를 하루
겹쳐 당일 추가 접수를 확인하며, 미완료 작업은 저장된 페이지부터 이어간다.
미리보기는 즉시 준비 작업으로 접수하고 공시 확인이 끝난 뒤 계산에 배정한다. 완료 알림
뒤 결과 읽기는 새 조회를 시작하지 않는다. 서버 재시작으로 끊긴 공시 확인 대기는 종료하고
사용자의 다음 시작을 기다린다. 초기 조회 범위와 완료 checkpoint를 혼동해
전체 과거 공시를 확인했다고 해석하지 않는다. 긴 중단이나 해석 불가 목록이 본문 전체연도
재수집 권한으로 바뀌지 않는다.

원문 대조는 페이지마다 누적 전체를 반복하지 않고 한 번의 조회 후 수행한다. 한 번의
대조에서 종목별 원문 요약을 재사용하고 작은 배치마다 이벤트 루프에 제어를 돌려준다.

설정 화면과 백테스트 위저드는 마지막 확인 시각·확인 완료 범위·최신성 경고를 읽는다.
목록 장애나 확인 이력 없음만으로 기존 검증 데이터의 실행을 막지 않는다. 확인된 손상·정정과
필수 입력 결손은 별도로 차단하며, 승인 전 자동 재시도하지 않는다. 실행 당시의 최신성 상태는
`provider_execution_provenance`에 고정한다. 승인·복구가 끝난 데이터는 새 revision·스냅샷으로
