...
1. **FK cascade 로 커버리지가 사라진다.** `symbolFactsState.code` 가 `symbols.code` 에 `onDelete: 'cascade'` 를 걸고 `removeSymbols` 가 실제로 그 행을 지운다. 종목을 뺐다 다시 넣으면 재무·자본변동 커버리지가 함께 사라져 이후 제출이 400 이다. 위저드는 재수집을 제안해 자가 복구되지만 복제 경로에는 그 카드가 없다. D-043 에 기록했다.
2. **e2e 가 게이트를 우회한다.** `seedCorporateActionCoverageOnRegistration` 이 등록되는 모든 종목을 무조건 통과시켜, 400 차단 경로와 위저드 게이트 화면이 e2e 커버리지 0 이다. 커버리지 없는 종목 하나만 남기면 최소 한 시나리오로 덮인다.
3. **`useBacktestLive` 에 같은 SSE 결빙 버그가 있다.** `api.ts:86-89` 가 `sseFailed` 뒤에도 낡은 SSE 값을 spread 로 우선한다 — 이 계획이 게이트 쪽에서 고친 것과 같은 결함이다. 백테스트 상세 진행률이 같은 방식으로 언다.
4. **`engine.ts` 가 역분할로 수량 0 이 된 대기 주문을 경고 없이 버린다.** 청산 주문이 사라지면 전략의 `exitPending` 이 참으로 굳어 상태가 어긋난다.
5. **`.tsx` 컴포넌트 테스트가 없다.** 순수 로직만 덮여 있어, 최종 리뷰가 잡은 위저드 경고 누락과 SSE 우선순위 결함이 둘 다 컴포넌트에 있었다.

### 그대로 둬도 되는 것

- 역분할 내림의 비결합성 — 정지 구간에 역분할이 둘 이상 겹칠 때만 닿는다. `engine.ts` 가 조건과 한계를 적어 뒀다
- 훅 게이트 확대로 `corporateActions()` 호출이 전 종목으로 늘었다 — 팩트 없는 종목은 빈 배열 조기 반환이다
- SSE 재연결 직후의 일시적 진행률 후퇴 — 얼어붙는 것보다 낫다

---

## 실행 회고 — 실패는 실행이 아니라 범위에서 났다

이 계획은 실행 중 여덟 번 틀렸고 전부 리뷰가 잡았다. 네 번이 같은 모양이었다 — **계획이 버그보다 좁은 표면을 지목했다.**

1. **Task 1** — `Math.floor(quantity * ratio)` 가 부동소수점 경계에서 한 주를 잃었다. `ratio` 를 정수로만 생각했는데 DART 주식수 두 개의 나눗셈이라 임의 유리수가 나온다. 내가 쓴 테스트 여섯 개가 그 경계를 하나도 안 밟았다
2. **Task 2** — "봉이 없는 종목은 드물다" 고 지시했다. KRX 는 주권교체 기간에 매매를 정지하므로 **분할 전후 봉 없음이 표준 경로**다. 고친 버그가 가장 흔한 경우에 그대로 남을 뻔했다
3. **Task 7** — "생성자에서 고아 행을 쓸어라" 는 지시 자체가 틀렸다. `createContainer()` 가 모든 CLI 서브커맨드에서 같은 DB 로 돈다. 리뷰어가 스크립트로 살아 있는 잡이 죽는 것을 실증했다
4. **최종 리뷰** — "전략 가격 상태" 를 `HoldingState` 세 필드로만 잡았다. 같은 항목이 `EmaState`·`AtrState`·`RsiState`·`RollingMax` 도 들고 있고 전부 원본 종가를 먹는다
5. **최종 리뷰** — `0008` 백필을 빠뜨렸다. 배포 첫날 모든 제출이 400 이 될 뻔했다

### 다음 계획에 옮길 것

- **"통과하는 테스트" 와 "그 버그를 잡는 테스트" 는 다르다.** 이 계획에서 세 번 나왔다 — 정지 테스트가 옛 코드에서도 통과했고, 진입 seed 는 커버리지가 0 이었고, 규칙 3 은 두 라운드 동안 무방비였다. **수정을 되돌려 깨지는 것을 보는 단계**를 브리프에 넣어야 한다
- **구현자가 "이건 좀 걸리는데" 로 올린 것은 대체로 진짜였다.** 행 존재 불변식, 첫 진입 BUY 공백, `clone-draft` 경고 누락, FK cascade — 전부 리뷰에서 실제 위험으로 확인됐다
- **한 상태가 여러 곳에 있는지 먼저 세라.** "가격 상태" 를 한 타입으로 봤는데 다섯 곳이었다
- `CLAUDE.md` 의 3줄 문장 규칙이 열두 번 넘게 어겨졌고 그중 여러 번이 **그 규칙을 고치는 커밋**에서 재발했다. 문장을 쓴 뒤 줄 수를 세는 단계를 명시해야 한다
