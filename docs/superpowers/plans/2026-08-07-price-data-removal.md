...
**정정 (2026-08-08, Task 6 리뷰에서 발견):** 위 코드는 등록 검사를 잃는다. 예전에는 `symbolService.getCoverage` 가 `symbols` 에 등록된 종목만 담아서, 미등록 종목이 제출 시점에 400 으로 걸렸다. `krx_daily_bars` 를 직접 읽으면 그 게이트가 사라져 요청이 큐에 들어갔다가 실행 단계에서 죽는다 — 큐 슬롯을 먹고 에러도 늦다. **`symbolService.exists` 검사를 명시적으로 넣어라** (사용자 결정 2026-08-08). 예전에는 캐시의 부작용으로 얻던 것을 이제 코드가 의도로 말해야 한다. `backtest-child.ts` 의 실행 시점 검사는 그대로 둔다 — 제출과 실행 사이에 종목이 빠질 수 있어 두 겹이 맞다.
