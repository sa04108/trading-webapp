...
- 워밍업: `correlationBars`(기본 60) 봉이 전 종목에 채워지는 첫 시점에 종목간
  ...
  - "채워진다" 는 **봉 시각 기준 공통 구간**이다. 중간 상장·거래정지로 종목별
    봉 개수가 다르면 배열 인덱스로 맞출 수 없다 — 한 봉만 밀려도 완전 역상관이
    정상관으로 뒤집혀 그룹이 조용히 깨진다.
  - 봉이 아예 없는 종목이 유니버스에 있으면 공통 구간이 채워지지 않아 그룹이
    확정되지 않는다. 그러면 **진입이 영영 없고 경고도 없다**(전략에 경고 채널이
    없다 — 거래 0건으로 끝난다). 봉 없는 종목은 유니버스에 넣지 않는다.
