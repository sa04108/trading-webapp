...
일일 호출 한도에 닿으면 DART·KRX 준비가 대기 상태로 남아 다음 KST 날짜에 자동으로 이어지고,
서버 재시작 뒤에도 중단된 지점부터 이어진다 — 실패가 아니다. 준비를 취소해도 그때까지
저장한 종목 데이터는 지우지 않는다. DART·KRX 실제 호출 수는
`external_api_daily_usage`에 KST 날짜별로 남아 재부팅 뒤에도 이어지고, 공급자가 한도
초과를 반환하면 알림 센터에 `data-sync/error` 알림이 영속된다.
