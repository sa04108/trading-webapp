...
## 2026-09-13 수집 결손 누적과 중단 후 원문 재사용

운영 `df68765`에서도 준비 자식의 128 MiB V8 OOM이 발생했다. 해당 실패 작업은 조사 시점에 이미 삭제되어 단계 상태를 복원할 수 없었고 힙 덤프도 없었다. 별도로 남아 있던 `SYNCING_FACTS` 한도 대기 작업은 212종목 중 88종목 처리 시 결손 280,924건을 기록했다. 기존 readiness 메모리 검증은 이 수집 경로를 포함하지 않았다.

`FactSyncService`는 연도별 coverage 저장 이후에도 전체 결손 객체와 사유 문자열을 `gaps` 배열에 보존했다. 종목당 약 3,190건의 합성 결손을 넣은 수정 전 서비스는 100종목에서 관측 힙 116 MiB에 도달한 뒤 같은 V8 OOM으로 종료했다. 이 재현은 누적 경로의 결함을 증명하지만 장애 당시 운영 힙과 동일한 입력이라는 뜻은 아니다.

수집 리포트는 이제 전체 `gapCount`와 최대 100건·사유 240자 이내의 예시를 분리한다. 연도별 coverage에는 예시 제한 전의 결손을 전부 전달하므로 뒤늦게 발견한 BLOCKING 결손과 원본 manifest 판정을 유지한다. 준비 작업의 UI·대기 상태는 예시 배열 길이 대신 전체 건수를 사용한다.

초기 수집의 중간 연도에서는 완료 watermark가 0으로 남는다. 종전 재시도는 이를 오래된 수집으로 취급해 이미 닫힌 연도도 REFRESH했고, 첫 연도를 닫기 전 실패한 경우에는 저장된 원문 전체를 무시했다. 이제 완료 이력이 부족한 종목은 SQLite에서 가장 이른 원문 수집 시각만 집계해 공시 확인의 하한으로 사용한다. 그 이후 새 공시가 없으면 저장된 원문을 재사용하고 누락·손상된 응답만 요청한다. 완료 watermark를 임의로 전진시키지는 않는다. 90일 이전 원문처럼 최신성을 확인할 수 없거나 새·정정공시가 확인된 연도, 명시적인 FULL 수집은 계속 원천에서 갱신한다.

재무 수집은 자본변동도 함께 저장하므로 처리한 재무 공시 접수번호를 후속 자본변동 수집에서도 참조해 같은 공시의 재요청을 피한다. 자본변동 전용 수집이 재무 완료 접수번호를 생성하지는 않는다. 공시 목록 `list.json`도 공통 DART 호출 경로를 사용해 페이지·HTTP 재시도의 실제 호출을 기록하고 공급자의 한도 응답을 `DartQuotaError`로 전달한다. 새 공시 확인을 위한 목록 조회는 재시도 시에도 필요할 수 있다.

회귀 검증:

- `tests/integration/fact-sync-memory.test.ts`: 재무·자본변동 각각 212종목·11년·결손 676,280건을 실제 128 MiB Node 자식에서 완료한다. 리포트 예시 100건, 전체 결손 수 및 BLOCKING 2,332건의 coverage 전달, RSS 320 MiB 미만을 확인한다.
- `tests/integration/dart-sync-resume.test.ts`: 실제 SQLite 연결과 어댑터를 다시 만들어 첫 연도 완료 전/후 중단을 재개한다. 재무 28개 응답 중 7개 또는 20개를 받은 뒤 중단하면 다음 날 각각 21개 또는 8개만 추가 요청하고 중단 없는 실행과 같은 facts를 만든다. 자본변동 전용 재개, 오래된 원문, 새 정정공시와 후속 자본변동 재사용도 검사한다.
- 단위 테스트는 예시 상한 이후 BLOCKING 결손 보존, 전체 건수 표시, 원문 시각 집계, 목록 API의 페이지·재시도 계수 및 한도 차단을 검사한다.

검증에는 합성 응답만 사용하며 실제 DART API는 호출하지 않는다. 운영 배포와 운영 요청 완료 확인은 이 로컬 회귀 검증에 포함하지 않는다.

최종 검증은 `pnpm exec vitest run --maxWorkers=2` 전체 176개 파일·2,010개 테스트 통과(589.53초), `pnpm typecheck`, `pnpm lint`, `pnpm build` 통과다. 웹 빌드에는 기존 500 kB 초과 번들 안내가 남아 있다.
