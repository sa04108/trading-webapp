...
- 토스증권 동기화 파이프라인 구현·와이어링 완료 (D-018·D-020) — TOSS_CLIENT_ID/SECRET 설정과 허용 IP 등록만 남음. 미설정 시 sync 잡이 CSV 안내와 함께 실패. 1h 봉 미제공(1분봉 집계로 생성), 증분 자동 스케줄러는 미구현(수동 sync 트리거)
