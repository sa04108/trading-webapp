...
- 배당·생존 편향 미보정 — 결과 화면 경고에 표시 (§9.4). 액면분할은 분할 이력이 수집된 데이터셋에서 신호 계산 시 보정되나(체결가는 항상 실제 거래 가격), 거시 지표 수집(`FactScope.MACRO`)·시점별 지수 구성·업종 분류·US 재무제표(SEC EDGAR)·`facts:sync` 웹 라우트화는 이번 범위가 아니다
