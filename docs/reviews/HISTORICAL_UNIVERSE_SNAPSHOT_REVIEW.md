...
> 기준일: 2026-08-02 \
> 상태: 구현 완료(v2 실행 종속 모델) — smoke test 게이트 대기 [^1] \
> 대상: 데이터셋 종목 구성, KRX 시장 데이터 연동, 백테스트 입력 검증 \
> 기준 커밋: `aeb4064` \
> 관련 검토: [BACKTEST_BIAS_REVIEW.md](./BACKTEST_BIAS_REVIEW.md)의 B-001·B-002·B-007 및 P-002

[^1]: 코드·마이그레이션·화면은 구현했지만(Task 1~16), §7 이 제안한 "데이터셋에
    provenance 를 붙이는" 소유 모델은 사용자 결정(2026-08-03)으로 "유니버스 스냅샷을
    독립 엔티티로 두고 백테스트 실행이 직접 참조하는" 실행 종속 모델로 바뀌었다 —
    자세한 이유와 §7.3 수동 편집 상태 기계가 필요 없어진 경위는
    [DECISIONS.md D-040](../DECISIONS.md#d-040-과거-시점-고정-유니버스-소유자를-데이터셋이-아니라-실행으로-바꾼다-review-7-데이터셋-결합안-대체)
    참고. §12 의 재개 조건 중 2(API별 이용 승인 완료 확인)·4(실응답 조인·분류 검증,
    `scripts/krx-smoke.ts` 실행)·5·6(비상업 약관 범위·로컬 저장·재사용·만료 후 처리
    확인)은 KRX 인증키가 없어 아직 사람이 확인하지 못했다 — 통과 전에는 이 기능을 실서비스에
    열지 않는다.
