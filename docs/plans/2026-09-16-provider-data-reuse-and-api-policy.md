...
후속 변경(2026-09-20): 이 문서의 「사용자 선택 확정」으로 적힌 범위별 수동 재수집 승인
요구는 사용자가 요청하거나 확정한 정책이 아니었으며 D-099가 대체한다. 필요한 최초 수집·정정·
손상 복구·필수 입력 보충은 자동 처리한다. 아래 수동 승인 관련 본문은 당시 계획 이력으로
보존하며, 현재 구현과 운영에는 D-099가 우선한다.
